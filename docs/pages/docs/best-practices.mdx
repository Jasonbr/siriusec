...
1. Select a branch from [github.com/siriusec/siriusec](https://github.com/siriusec/siriusec/) corresponding to the version of the documentation/product you'd like to work on.
2. Use `$ git submodule update --init --recursive` when you `git switch` to or `git checkout` a new branch!
3. Make changes locally using [Next](https://github.com/siriusec/next), our primary documentation tool.
4. Use an editing and (Flesch-Kincaid) conversational scoring tool like Grammarly.
5. Validate your changes locally using the provided [linters](https://github.com/siriusec/next#development-related-commands).
...
To see the current submodule and version mappings review [the mappings](https://github.com/siriusec/next/blob/main/.gitmodules).

## Tooling

Our documentation is primarily built with [Next](https://github.com/siriusec/next) which has the following features:
...
- Use [Siriusec's lucidchart library](https://app.lucidchart.com/lucidchart/dfcf1f4a-5cf0-4758-8ebb-f6ea86900aba/edit) to create diagrams with a consistent design language.
- Diagram multistep sections using sequence diagrams that depicts steps linearly.
- Several great examples are available here: https://siriusec.slab.com/posts/diagrams-ix9nzhpd.
...
<video autoPlay loop muted playsInline>
  <source src="https://gosiriusec.com/siriusec/videos/database-access-preview/dbaccessdemo.mp4" type="video/mp4" />
  <source src="https://gosiriusec.com/siriusec/videos/database-access-preview/dbaccessdemo.webm" type="video/webm" />
...
Some standards are presently codified in the [Next](https://github.com/siriusec/next) repository by way of MDX linters. Make sure to use these!
...
8. Article titles should follow a `Title Name | Siriusec Docs` format. The total character count should not exceed 70 for the entire title since this can impact SEO. **The suffix** (`| Siriusec Docs`) **will be added automatically by the Next engine** but keep a lookout for character count and that titles correctly render. 
...
12. Avoid scare-quotes when describing something - just describe it. `Siriusec is a way to "access stuff"` vs. `Siriusec represents an evolution of SSH`.
