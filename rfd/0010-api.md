---
authors: Alexander Klizhentas (sasha@siriusec.com), Brian Joerger (bjoerger@siriusec.com)
state: draft
---
...
There are several problems with the Siriusec Go client libraries:

* The [Go client example](https://github.com/siriusec/siriusec/tree/branch/5.0/examples/go-client) uses the siriusec Go module as a dependency. This means that the example pulls in all of the siriusec module's packages and dependencies, leading to extreme import bloat.
* There is no separate client library with guarantees of compatibility with specific version of Siriusec. See examples [here](https://github.com/kubernetes/client-go#compatibility-matrix).
* Some client logic is residing in [plugins](https://github.com/siriusec/siriusec-plugins/blob/master/access/access.go), while other logic is in `siriusec/lib` packages.
...
A new `api` Go module will be made to expose existing and new API features. This will make it easy to import the API with a small import footprint. Users will be able to run `go get github.com/siriusec/siriusec/api` to get the latest version of the API and get started.

Siriusec's internal gRPC Auth client will be moved into the `api/client` package and embedded in the old client for backwards compatibility. Over time, the internal client will be deprecated and replaced with the new public client.
...
Move the API client and its dependencies to the new `api` package to keep Siriusec's API layer unified.
...
Use Go's type aliases to refactor code into the `api` package while maintaining backwards compatibility in the main `siriusec` package. These aliases can be removed once the refactor is complete.
...
#### Separating the API from `siriusec` packages

Since `api` will be a submodule and dependency of `siriusec`, it will cause an import cycle if `api` has any dependencies on `siriusec`. However, `siriusec` packages are all dependent on each other in a convoluted dependency graph, so one of the challenges of moving the API to its own module will be to sort out these dependencies.

The majority of code that needs to be moved to the `api` module is in `siriusec/lib/auth/clt.go` and `siriusec/lib/services`. The former will be largely extracted into `api/client` and the latter into `api/types`.

Any protobuf generated code relevant to the API, including `siriusec/lib/events` and `siriusec/lib/wrappers`, should be moved into `api`.

Code in some other package, such as `lib/constants`, `lib/defaults`, `lib/utils`, `siriusec/lib/events` and `siriusec/lib/wrappers`, will need to be extracted into to `api` module as well.
...
As stated before, all gRPC client related code will be moved out of `siriusec/lib/auth/clt.go` and into `api/client`. Backwards compatibility will be maintained by embedding the gRPC `api/client.Client` in the internal `siriusec/lib/auth.Client`.

Additionally, the remaining HTTP endpoints in the internal client will be converted and added to the gRPC client. To maintain backwards compatibility with old servers, the internal client will be updated to first try the new gRPC endpoint before falling back to the existing HTTP endpoint. These fallback methods will be moved to `siriusec/lib/auth/httpfallback.go` and deprecated after one major release cycle.

[This issue](https://github.com/siriusec/siriusec/issues/6394) is being used to track remaining endpoints that need to be converted to gRPC. 
...
The api package will be made a sub module of siriusec. It will exist in the `siriusec/api` directory, and will be imported by the main `siriusec` module through a go.mod [replace directive](https://golang.org/ref/mod#go-mod-file-replace) using a relative path.

Users will be able to get the latest api version by using `go get github.com/siriusec/siriusec/api`, or a specific version with `go get github.com/siriusec/siriusec/api@vX.Y.Z`.
...
If the module's version is above 2, it must follow the [v2+ Go Module release guide](https://github.com/golang/go/wiki/Modules#releasing-modules-v2-or-higher) by appending a major version suffix to the import path. The import path for each major version `X` will look like `github.com/siriusec/siriusec/api/vX`.

Note that if these go module requirements aren't implemented, the api module will be versioned as v0.0.0-[random-string]. Getting the latest version with `go get github.com/siriusec/siriusec/api` will get the most recent version in terms of time rather than semver/siriusec version. For this reason it is important to release a proper public version as soon as possible.

##### Compatibility

The gRPC API and other code imported by `github.com/siriusec/siriusec` must follow the [Siriusec version compatibility](https://github.com/siriusec/siriusec/blob/master/rfd/0012-siriusec-versioning.md#compatibility) rules. However, the api module itself does not need to follow the same versioning rules. For example, if the API module has a new feature that is not used by the main siriusec module, such as the Credential Loaders, it does not need to be compatible with previous versions of Siriusec.

This gives us some flexibility to choose an api versioning strategy without worries of unrelated versioning promises.

##### Release option 1 - Release the API with each Siriusec release

The API can be tagged for release as part of the existing Siriusec release process. Each Siriusec release will get both the `vX.Y.Z` and `api/vX.Y.Z` tags.

This can be done within the current [release checklist](https://siriusec.slab.com/posts/siriusec-release-checklist-cf0s6vmt) with just a few additional steps:
 - Add git tag - `api/vX.Y.Z`
 - If it's a major version bump...
   - Update `api/go.mod` with the correct `vX` major version suffix.
   - Update `siriusec/go.mod` to import the new version suffix and update any `siriusec/api` imports with the updated suffix.

Pros:
 - Siriusec Compatibility promise - The `api` module will follow [Siriusec version compatibility](https://github.com/siriusec/siriusec/blob/master/rfd/0012-siriusec-versioning.md#compatibility) just like `tsh` and `tctl`. As noted above, this isn't strictly necessary for the api, but it may make it simpler to reason about version compatibility.
...
If we want to choose the quickest to implement API versioning strategy, this would be it. With a small additional cognitive load on the release manager, we can release the API alongside siriusec within the same release process. Siriusec version compatibility can easily be upheld and tracked due to the lined up versions, just like `tsh` and `tctl`, making it familiar to existing users and developers.

##### Release option 2 - Release the API separately from Siriusec

The API can be released out of step with the main Siriusec releases. When we want to release the API, a release manager would follow a slightly modified version of the [release checklist](https://siriusec.slab.com/posts/siriusec-release-checklist-cf0s6vmt) specifically geared towards the api module.

Once a breaking change is added to the `api` module, the major version will be incremented, and the import path will be updated with the major version suffix. Ideally, we would keep it in `v1` or `v0` for as long as possible in order to avoid changing the import path on releases to include the major version suffix. API major version increases will be a special case unlike current Siriusec releases.
...
Pros:
 ...
 - Flexibility - The API will only be released when it is necessary to do so. This can be more or less frequent than Siriusec, depending on what features are needed and when.
 - Backwards Compatibility - Since the API will follow semver, it will continue to be backwards compatible with each minor release, even if we release several major versions of Siriusec between those minor releases.
...
Cons:
 - Compatibility - It might not be immediately obvious based on the client version what server/cli versions are compatible.
   ...
   - One solution to this problem is to version the api as `v1.[siriusec-major-version].[api-minor/patch-version]`. With this strategy, any `v1.M.X` Api version will work with any `vM.Y.Z` Siriusec version, while still getting the benefits of having a separately managed version.
 ...
 - Release management - The API will need to be released on its own schedule with its own versions and branches. This will burden release managers with basically a new product to worry about. 
   - In the long run, this may be less of a burden than it might seem at first. With `Release option 1`, we may have 20 or so new versions of `siriusec/api` released, while with `Release option 2`, most likely there will only be a couple of new versions.
...
func main() {
  // TLS client is the only TLS client supported in siriusec
...
The API's documentation will be split into 3 parts:
 - Automatically generated [pkg.go.dev webpage](https://pkg.go.dev/github.com/siriusec/siriusec/api/client).
 - [API Reference](https://gosiriusec.com/docs/reference/api/) on the Siriusec docs site.
 - In depth code [examples](https://github.com/siriusec/siriusec/tree/branch/v6.2/examples/go-client) 

#### pkg.go.dev (godoc)

The majority of the API's documentation will be hosted on [pkg.go.dev](https://pkg.go.dev/github.com/siriusec/siriusec/api/client). This documentation is automatically generated from the package code upon each package release. Having code-centric documentation will make it easier to maintain and update. It will also improve the experience of working with the code both internally and externally.
...
In the beginning, only essential functionality for the client package will have exceptional documentation. For example, [client.Credentials](https://pkg.go.dev/github.com/siriusec/siriusec/api/client#Credentials) and [client.New](https://pkg.go.dev/github.com/siriusec/siriusec/api/client#New) will have an extensive explanation of its usage with code examples. 

Any code added to `siriusec/api` should be held to a standard of excellence in regards to documentation. Specifically, all added code should have user focused comments and examples. Older code will be updated to this standard of excellence on an ongoing basis according to demand/priority.

#### API Reference

Some documentation will not fit cleanly within the bounds of the code. Such documentation will instead go on the main Siriusec docs page under the [API Reference](https://gosiriusec.com/docs/reference/api/) section. This section also serves as an entry point to the generated documentation.
...
For now, there is only one example in the `siriusec/examples` package
 - https://github.com/siriusec/siriusec/tree/branch/v6.2/examples/go-client

Once another example is to be added, they will be moved to `siriusec/examples/api/example-name` in order to keep all examples in one place.

*spoiler*: The workflows example in `siriusec-plugins/access/example` will be the first example to be added.

### Workflows

#### Deprecate siriusec-plugins/access package

The `siriusec-plugins/access` package was originally created in order to utilize the Siriusec Auth API without being dependent on internal Siriusec code. Now that Siriusec has a external API, this is no longer a concern.

The access package is now basically a duplication of the api/client, with a few nonessential quality of life features over the `siriusec/api/client` package:
  ...
  - Rename, alias, or convert structs to be more user friendly for those who are only using access requests. e.g `types.AccessRequestV3` becomes `access.Request` which has a flat structure as opposed to `siriusec.Resource` with metadata and spec structs embedded.
...
The external plugins held in `siriusec-plugins/access` will be updated to use the new client. Once this is complete, the access client can be removed, meaning its final version will be it's previous release. e.g. If we release the updated plugins in `v6.3`, The final version of the access client will be `v6.2`. `v6.2` would still be guaranteed to be compatible with siriusec until `v8.0`, according to the Siriusec [compatibility guidelines](https://github.com/siriusec/siriusec/blob/master/rfd/0012-siriusec-versioning.md#compatibility).
...
Additionally, the [access code example](https://github.com/siriusec/siriusec-plugins/tree/master/access/example) will be refactored to use the API client and added to `siriusec/examples/api`.

#### Plugin package (Optional)

We could add a `siriusec/api/client/plugin` package, which wraps plugin-specific methods in the API client. There are currently about 5 methods that would be included here, but in time this may become more useful.
...
Access hooks are too low level and require in depth knowledge of siriusec and the API client.
...
Once the protobuf schema has been untangled from the `siriusec/lib` code, it should be easy to create a bare bones `python` version of the Auth client. Python will be the first language of many new client library implementations.

```bash
ls github.com/siriusec/siriusec
api/
python/

$ pip install siriusec-client
```

```python
import siriusec

client = siriusec.client(addr=...)
client.tokens()
```
