...
import (
	...
	"github.com/siriusec/siriusec/lib/config"
	...
	"github.com/siriusec/siriusec/lib/sshutils/scp"
...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *service.Config) {
	...
	isDebug, _ := strconv.ParseBool(os.Getenv(siriusec.VerboseLogsEnvVar))
	if isDebug {
		trace.SetDebug(true)
	}
	...
	dump.Flag("output",
		...
		siriusec.SchemeStdout).StringVar(&dumpFlags.output)
	dump.Flag("acme",
...
func OnStart(config *service.Config) error {
	return service.Run(context.Background(), *config, nil)
}
...
func onStatus() error {
	...
	siriusecUser := os.Getenv(siriusec.SSHSiriusecUser)
	proxyHost := os.Getenv(siriusec.SSHSessionWebproxyAddr)
	clusterName := os.Getenv(siriusec.SSHSiriusecClusterName)
	hostUUID := os.Getenv(siriusec.SSHSiriusecHostUUID)
	sid := os.Getenv(siriusec.SSHSessionID)
...
func (flags *dumpFlags) CheckAndSetDefaults() error {
	if flags.testConfigFile != "" && flags.output != siriusec.SchemeStdout {
		return trace.BadParameter("only --output or --test can be set, not both")
	}
	if flags.output == "" || flags.output == siriusec.SchemeFile {
		flags.output = siriusec.SchemeFile + "://" + defaults.ConfigFilePath
	} else if flags.output == siriusec.SchemeStdout {
		flags.output = siriusec.SchemeStdout + "://"
	}
	return nil
}
...
func onConfigDump(flags dumpFlags) error {
	...
	case siriusec.SchemeFile, "":
		...
		f, err := os.OpenFile(uri.Path, os.O_RDWR|os.O_CREATE|os.O_EXCL, siriusec.FileMaskOwnerOnly)
...
// onSCP implements handling of 'scp' requests on the server side. When the siriusec SSH daemon
// receives an SSH "scp" request, it launches itself with 'scp' flag under the requested
// user's privileges
//
// This is the entry point of "siriusec scp" call (the parent process is the siriusec daemon)
func onSCP(scpFlags *scp.Flags) (err error) {
	// when 'siriusec scp' is executed, it cannot write logs to stderr (because
	// they're automatically replayed by the scp client)
	utils.SwitchLoggingtoSyslog()
	if len(scpFlags.Target) == 0 {
		return trace.BadParameter("siriusec scp: missing an argument")
	}
...
func onExec() error {
	srv.RunAndExit(siriusec.ExecSubCommand)
	return nil
}
...
func onForward() error {
	srv.RunAndExit(siriusec.ForwardSubCommand)
	return nil
}
