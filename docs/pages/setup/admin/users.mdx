...
Siriusec's local user accounts are created and stored in Siriusec's internal storage.
...
A user identity in Siriusec exists in the scope of a cluster.
A Siriusec administrator creates Siriusec user accounts and maps them to the roles they can use.
...
| bob | bob | Siriusec user 'bob' can log in into member nodes only as OS user 'bob' |
| ross | | If no OS login is specified, it defaults to the same name as the Siriusec user - 'ross'. |

Let's add a new user to Siriusec using the [`tctl`](../reference/cli.mdx#tctl) tool:

```code
$ tctl users add joe --logins=joe,root --roles=access,editor
```

Siriusec generates an auto-expiring token (with a TTL of 1 hour) and prints the
