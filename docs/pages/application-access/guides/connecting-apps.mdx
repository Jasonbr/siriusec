---
title: Use Siriusec for Application Access
description: How to configure Siriusec for Application Access.
---

# Connecting Web Applications

Download the latest version of Siriusec for your platform from our [downloads page](https://gosiriusec.com/siriusec/download)
and follow the installation [instructions](../../installation.mdx).

## Start Auth/Proxy service

Create a configuration file for a Siriusec service that will be running
...
proxy_service:
  ...
  public_addr: siriusec.example.com:3080
ssh_service:
  enabled: "no"
```
...
```code
$ sudo siriusec start --config=/path/to/siriusec.yaml
```

### Generate a token

A join token is required to authorize a Siriusec Application Proxy service to
...
TLS is required to secure Siriusec's Access Plane and any connected
applications. When setting up Siriusec, the minimum requirement is a certificate
for the proxy and a wildcard certificate for its sub-domain. This is where
everyone will log into Siriusec.

In our example:

- `siriusec.example.com` will host the Access Plane.
- `*.siriusec.example.com` will host all of the applications e.g. `grafana.siriusec.example.com`.

Siriusec can obtain a certificate automatically from Let's Encrypt using
...
A Siriusec user needs their role's permission to access an application. Siriusec
comes with a built-in `access` role that grants access to all apps:

```code
$ tctl --config=/path/to/siriusec.yaml users add --roles=access appuser
```

## Start application service with CLI flags

A Siriusec Application Proxy agent can be started with a single CLI command:
...
Note that the `--auth-server` flag must point to the Siriusec cluster's proxy
...
After Siriusec is running, users can access the app at `app-name.proxy_public_addr.com`
e.g. `grafana.siriusec.example.com`. You can also override `public_addr` e.g
`grafana.acme.com` if you configure the appropriate DNS entry to point to the
Siriusec proxy server.

## Start application service with a config file

Example `siriusec.yaml` configuration:
...
siriusec:
  ...
  # tunnel.
  auth_servers:
  - siriusec.example.com:3080
app_service:
    enabled: yes
    # Siriusec provides a small debug app that can be used to make sure application
    ...
    - name: "grafana"
      ...
      public_addr: "grafana.siriusec.example.com"
...
Siriusec checks if the certificates presented by the applications are signed
...
rewritten: `X-Siriusec-Jwt`, `Cf-Access-Token`, `X-Forwarded-*`.
...
## View applications in Siriusec

Siriusec provides a UI for quickly launching connected applications.
...
do so by hitting the `/siriusec-logout` endpoint:

- `https://internal-app.siriusec.example.com/siriusec-logout`
