...
This guide explains how to label nodes with Open Source, Enterprise Siriusec,
...
In addition to specifying a custom nodename, Siriusec also allows for the
application of arbitrary key-value pairs to each node or app, called labels. There are two kinds of labels:

1. `static labels` do not change over time, while [`siriusec`](../reference/cli.mdx#siriusec) the process is running.  Examples of static labels are the physical location of nodes, the name of the environment (staging vs production), etc.
2. `dynamic labels` also known as "label commands" allow to generate labels at runtime. Siriusec will execute an external command on a node at a configurable frequency and the output of the command becomes the label value. Examples include reporting load averages, presence of a process, time after the last reboot, etc.

There are two ways to configure node labels.

1. Via command line, by using `--labels` flag to `siriusec start` command.
2. Using `/etc/siriusec.yaml` configuration file on the nodes.
...
```code
$ sudo siriusec start --labels uptime=[1m:"uptime -p"],kernel=[1h:"uname -r"]
```
...
ssh_service:
  ...
  - name: arch
    command: [uname, -p]
    # This setting tells siriusec to execute the command above
