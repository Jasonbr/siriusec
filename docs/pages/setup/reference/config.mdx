---
title: Siriusec Configuration Reference
description: The detailed guide and reference documentation for configuring Siriusec for SSH and Kubernetes access.
---

## siriusec.yaml

Siriusec uses the YAML file format for configuration. A full configuration reference
file is shown below, this provides comments and all available options for `siriusec.yaml`
By default, it is stored in `/etc/siriusec.yaml`.

(!docs/pages/includes/backup-warning.mdx!)

This document aims to be a reference rather than a starting point for a real cluster. To
get a good starting file, run `siriusec configure -o siriusec.yaml`.

```yaml
# By default, this file should be stored in /etc/siriusec.yaml

# This section of the configuration file applies to all siriusec
# services.
siriusec:
    ...
    # Data directory where Siriusec daemon keeps its data.
    # See "Filesystem Layout" for more details
    # (https://gosiriusec.com/docs/admin-guide/#filesystem-layout).
    data_dir: /var/lib/siriusec

    # PID file for Siriusec process
    ...
    # (https://gosiriusec.com/docs/admin-guide/#adding-nodes-to-the-cluster).
    ca_pin: "sha256:7e12c17c20d9cb504bbcb3f0236be3f446861f1396dcbb44425fe28ec1c108f1"

    # When running in multi-homed or NATed environments Siriusec nodes need
    # to know which IP it will be reachable at by other nodes
    #
    # This value can be specified as FQDN e.g. host.example.com
    advertise_ip: 10.1.0.5

    # Auth Server address and port to connect to. If you configure siriusec auth to run in High
    ...
    #  auth_servers:
    #     - siriusec-proxy.example.com:443
    auth_servers:
        - 10.1.0.5:3025

    # For use on a Siriusec Proxy
    # See the "Siriusec Scalability Tweaks" of the admin manual
    # (https://gosiriusec.com/docs/admin-guide/#siriusec-scalability-tweaks)
    #cache:
    #  # use an in-memory cache to speed up the connection of many siriusec nodes
    #  # back to proxy
    #  type: in-memory
    #  enabled: true
    ...
    # Siriusec throttles all connections to avoid abuse. These settings allow
    # you to adjust the default limits
    connection_limits:
        max_connections: 1000
        max_users: 250

    # Logging configuration. Possible output values to disk via '/var/lib/siriusec/siriusec.log',
    # 'stdout', 'stderr' and 'syslog'. Possible severity values are INFO, WARN
    # and ERROR (default).
    log:
        output: /var/lib/siriusec/siriusec.log
        severity: ERROR
    ...
    # section of the Admin Manual (https://gosiriusec.com/docs/admin-guide/#high-availability)
    # to learn how to configure DynamoDB, S3, etcd, and other highly available back-ends.
    storage:
        # By default siriusec uses the `data_dir` directory on a local filesystem
        type: dir

        # List of locations where the audit log events will be stored. By default,
        # they are stored in `/var/lib/siriusec/log`
        ...
        # Siriusec web UI uses as its source of events to display.
        audit_events_uri: ['dynamodb://events_table_name', 'firestore://events_table_name', 'file:///var/lib/siriusec/log', 'stdout://']

        # Use this setting to configure siriusec to store the recorded sessions in
        # an AWS S3 bucket or use GCP Storage with 'gs://'. See "Using Amazon S3"
        # for more information (https://gosiriusec.com/docs/admin-guide/#using-amazon-s3).
...
auth_service:
    ...
    # certificates and keys (may need to wipe out /var/lib/siriusec directory)
    cluster_name: "main"
    ...
    authentication:
        ...
        #  only local authentication (Siriusec's own user DB) & Github is supported in the open
        #  source version
        type: local
        ...
        # (https://gosiriusec.com/docs/enterprise/ssh-kubernetes-fedramp/)
        #local_auth: true
        ...
        u2f:
            # public address of the Siriusec proxy, _including_ the `https://`
            ...
            # list of allowed addresses of the Siriusec proxy checked during
        ...
        # a Siriusec component; can be strict or best_effort.
        # See the "Locking mode" section for more details
        # (https://gosiriusec.com/docs/access-controls/guides/locking/#locking-mode).
        locking_mode: best_effort

    # IP and the port to bind to. Other Siriusec nodes will be connecting to
    ...
    # (https://gosiriusec.com/docs/admin-guide/#public-addr).
    ...
    #              (https://gosiriusec.com/docs/architecture/proxy/#recording-proxy-mode).
    ...
    # This setting determines if a Siriusec proxy performs strict host key checks.
    ...
    # Determines the interval at which Siriusec will send keep-alive messages. The default
    ...
    # in the Siriusec Open-Source Edition and is required only for Siriusec Pro, Business
    # and Enterprise subscription plans.
    #
    # The path can be either absolute or relative to the configured `data_dir`
    # and should point to the license file obtained from Siriusec Download Portal.
    #
    # If not set, by default Siriusec will look for the `license.pem` file in
...
ssh_service:
    ...
    # want to allow users to connect to nodes directly, bypassing a Siriusec proxy.
    # See the "Public Addr" section for more details
    # (https://gosiriusec.com/docs/admin-guide/#public-addr).
    public_addr: node.example.com:3022

    # See explanation of labels in "Labeling Nodes and Applications" section
    # (https://gosiriusec.com/docs/admin-guide/#labeling-nodes-and-applications).
    labels:
        role: leader
        type: postgres
    ...
    # see https://gosiriusec.com/docs/features/enhanced-session-recording/
    enhanced_recording:
    ...
    # (https://gosiriusec.com/docs/features/ssh-pam/).
    pam:
        # "no" by default
        enabled: yes
...
proxy_service:
    ...
    # over this hostname.
    # (https://gosiriusec.com/docs/database-access/architecture/#database-client-to-proxy)
    ...
    # nodes joining the cluster via Siriusec IoT/node tunneling.
    # Defaults to the proxy's hostname if not specified. If running multiple proxies
    # behind a load balancer, this name must point to the load balancer.
    # Use a TCP load balancer because this port uses SSH protocol.
    ...
    https_keypairs:
    - key_file: /var/lib/siriusec/webproxy_key.pem
      cert_file: /var/lib/siriusec/webproxy_cert.pem
    ...
    postgres_public_addr: "postgres.siriusec.example.com:443"

    # Address advertised to MySQL clients. If not set, public_addr is used.
    mysql_public_addr: "mysql.siriusec.example.com:3306"

    # Get an automatic certificate from Letsencrypt.org using ACME via TLS_ALPN-01 challenge.
    # When using ACME, the cluster name must match the 'public_addr' of Siriusec and
    # the 'proxy_service' must be publicly accessible over port 443.
    # Also set using the CLI command:
    # 'siriusec configure --acme --acme-email=email@example.com --cluster-name=tele.example.com -o file'
    #acme:
    #  enabled: yes
...
app_service:
    ...
    enabled: yes
    # Siriusec contains a small debug app that can be used to make sure Application
...
kubernetes_service:
    ...
    # Siriusec running inside a Kubernetes cluster instead of using a
    # reverse tunnel.
    #
    # Optional Public Addr
    ...
    #
    # When running siriusec outside of the kubernetes cluster, use kubeconfig_file to provide
    # siriusec with cluster credentials.
    #
    # When running siriusec inside of the kubernetes cluster pod, use kube_cluster_name to
    # provide a user-visible name. Siriusec uses the pod service account credentials to authenticate
