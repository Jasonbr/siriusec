## GCS Storage Implementation for Siriusec

### Introduction

This package enables Siriusec auth server to store session recordings in 
...
compile Siriusec with `gcs` build flag.

To build Siriusec with GCS enabled, run:

```
ADDFLAGS='-tags gcs' make siriusec
```

### Quick Start

Configuration options are passed to the GCS handler via a URI/URL. The following is a sample
configuration in `siriusec` section of the config file (by default it's `/etc/siriusec.yaml`):

```yaml
siriusec:
  storage:
    audit_sessions_uri: 'gs://siriusec-session-storage-2?projectID=gcp-proj&credentialsPath=/var/lib/siriusec/gcs_creds'
```
...
where Siriusec is installed on GCE instances and have service accounts with IAM role/profile
