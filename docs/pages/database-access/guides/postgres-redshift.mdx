...
description: How to configure Siriusec Database Access with AWS Redshift PostgreSQL.
...
Siriusec uses [IAM tokens](https://docs.aws.amazon.com/redshift/latest/mgmt/generating-user-credentials.html)
to authenticate with Redshift databases. IAM authentication is enabled by default
for all Redshift databases.

In order to authorize Siriusec to generate temporary IAM tokens, create an IAM
...
The `DescribeClusters` permission is required to allow Siriusec to retrieve
...
<Admonition type="note" title="Users auto-create">
  Siriusec does not currently use auto-create option when generating tokens so
...
Siriusec Database Access for Redshift is available starting from `6.2` release.
...
Create a local Siriusec user with the built-in `access` role:
...
Siriusec Database Service must have an IAM role allowing it to generate IAM
auth tokens which we created in the [Create IAM Policy](#create-iam-policy)
section above.

Siriusec uses the default credential provider chain to find AWS credentials.
