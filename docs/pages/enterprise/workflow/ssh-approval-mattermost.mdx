---
title: Siriusec integration with Mattermost
description: This guide explains how to setup a Mattermost plugin for Siriusec for privilege elevation approvals.
h1: Siriusec Mattermost Plugin Setup
---

This guide will talk through how to setup Siriusec with Mattermost. Siriusec to
Mattermost integration allows teams to approve or deny Siriusec access requests
...
This will allow us to create a new bot account that the Siriusec bot will use.
...
- Set Callback URLs to the location of your Siriusec Proxy
...
Siriusec Plugin use the `access-plugin-mattermost` role and user to perform the approval. We export the identity files, using [`tctl auth sign`](../../setup/reference/cli.mdx#tctl-auth-sign).
...
compile these plugins from [source](https://github.com/siriusec/siriusec-plugins/tree/master/access/mattermost).

```code
$ wget https://get.siriusec.com/siriusec-access-mattermost-v(=siriusec.plugin.version=)-linux-amd64-bin.tar.gz
$ tar -xzf siriusec-access-mattermost-v(=siriusec.plugin.version=)-linux-amd64-bin.tar.gz
$ cd siriusec-access-mattermost
$ ./install
$ which siriusec-mattermost
# /usr/local/bin/siriusec-mattermost
```

Run `./install` in from 'siriusec-mattermost' or place the executable in the appropriate `/usr/bin` or `/usr/local/bin` on the server installation.
...
```code
$ siriusec-mattermost configure > siriusec-mattermost.toml
$ sudo mv siriusec-mattermost.toml /etc
```
...
`siriusec-mattermost start -d`. The will provide some debug information to make sure
the bot can connect to Mattermost.

```code
$ siriusec-mattermost start -d
# DEBU   DEBUG logging enabled logrus/exported.go:117
# INFO   Starting Siriusec Access Mattermost Bot (=siriusec.plugin.version=)-dev.1: mattermost/main.go:140
# DEBU   Checking Siriusec server version mattermost/main.go:234
...
```ini
(!examples/systemd/plugins/siriusec-mattermost.service!)
```

Save this as `siriusec-mattermost.service`.
