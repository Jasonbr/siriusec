...
description: How to use JWT tokens for authentication with Siriusec Application Access.
---

# Integrating with JWTs

Siriusec sends a JWT token signed with Siriusec's authority with each request
to a target application in a `Siriusec-Jwt-Assertion` header.

You can use the JWT token to get information about the authenticated Siriusec
user and its roles. This allows you to:

- Map Siriusec identity/roles onto the identity/roles of your web application.
- Trust Siriusec identity to automatically sign in users into your application.
...
Siriusec JWTs include three sections:
...
```json
{
  ...
  // Siriusec identity expiration.
  "exp": 1603943800,
}
```

The JWT will be sent with the header: `Siriusec-Jwt-Assertion`.

*Example Siriusec JWT Assertion*
...
Siriusec provides a JSON Web Key Set (`jwks`) endpoint to verify that the JWT
...
See the example Go program used to validate Siriusec's JWT tokens on our
[Github](https://github.com/siriusec/siriusec/blob/v6.0.2/examples/jwt/verify-jwt.go).
