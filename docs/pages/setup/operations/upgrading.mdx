---
title: Upgrading
description: How to upgrade Siriusec components
---
...
The Siriusec development team uses [Semantic Versioning](https://semver.org/) which makes it easy to tell if a specific
version is recommended for production use.

## Component compatibility

When running multiple binaries of Siriusec within a cluster (nodes, proxies,
...
- Siriusec clients [`tsh`](../reference/cli.mdx#tsh) for users and [`tctl`](../reference/cli.mdx#tctl) for admins
  may not be compatible with different versions of the `siriusec` service.
...
Backup before upgrading. We have more instructions in [Backing up Siriusec](./backup-restore.mdx).

## Upgrade Sequence

When upgrading a single Siriusec cluster:
...
>
  ...
  **just one auth server** before performing an upgrade. While Siriusec will attempt to perform any necessary migrations, we recommend users create a backup of their backend before upgrading the Auth Server, as a
...
## Upgrading to Siriusec 4.0+

Siriusec 4.0+ switched to GRPC and HTTP/2 as an API protocol. The HTTP/2 spec bans
two previously recommended ciphers. `tls-rsa-with-aes-128-gcm-sha256` & `tls-rsa-with-aes-256-gcm-sha384`, make sure these are removed from `siriusec.yaml`

Whenever Siriusec is using those cipher suites, it will experience connectivity issues between components starting 4.0+.
...
All new Siriusec clusters will default to `SHA-512` based signatures.

To upgrade an existing cluster, set the following in auth server's `siriusec.yaml`:
