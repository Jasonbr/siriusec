...
Siriusec's default [SSH and Kubernetes session recording](../../architecture/nodes.mdx#session-recording)
feature captures what is echoed to a terminal.

This has inherent advantages, for example, because no input is captured, Siriusec
...
Siriusec Enhanced Session Recording mitigates all three concerns by providing advanced security, greater logging capabilities, and better correlates a user with their activities.

## Prerequisites

Siriusec 7.0+ with Enhanced Session Recording requires Linux kernel 5.8 (or above).
...
## Step 1/3. Install and configure Siriusec node

Follow our [installation instructions](../../installation.mdx) to install Siriusec Auth, Proxy
and Nodes.

Set up the Siriusec node with this `etc/siriusec.yaml`. See our [configuration file setup](../../setup/reference/config.mdx) for more instructions.

```yaml
# Example config to be saved as etc/siriusec.yaml
siriusec:
  nodename: graviton-node
  auth_token: exampletoken
  auth_servers:
  # Replace with IP of Siriusec Auth server.
  - 127.0.0.1:3025
...
## Step 2/3. Test by logging into node via Siriusec
...
The resulting enhanced session recording will be shown in [Siriusec's Audit Log](../../architecture/authentication.mdx#audit-log).

```code
$ siriusec-auth ~:  tree /var/lib/siriusec/log
...
# ├── events.log -> /var/lib/siriusec/log/1048a649-8f3f-4431-9529-0c53339b65a5/2020-01-13.00:00:00.log
# ├── playbacks
# │   └── sessions
# │       └── default
# └── upload
...
`tail -f /var/lib/siriusec/log/events.log`, the resulting capture from Siriusec will
