...
h1: Siriusec Access Controls Reference
---

This document describes Siriusec Roles, Role Options, and Role-Based Access Control (RBAC) for Siriusec Access Controls.

## Roles

A Siriusec `role` works by having two lists of rules: `allow` rules and `deny` rules.
...
To manage cluster roles, a Siriusec administrator can use the Web UI or the command
line using [tctl resource commands](../setup/reference/resources.mdx). To see the list of
roles in a Siriusec cluster, an administrator can execute:
...
spec:
  ...
  allow:
    ...
    # Needed for siriusec's access request workflow
    # https://gosiriusec.com/siriusec/docs/enterprise/workflow/
    request:
      roles:
      - dba
...
| - | - |
| `{{internal.logins}}` | Substituted with "allowed logins" parameter used in `tctl users add [user] <allowed logins>` command. This applies only to users stored in Siriusec's own local database. |
...
Both variables above are there to deliver the same benefit: they allow Siriusec
...
| `max_connections` | Limit on how many active SSH sessions can be started via Siriusec | |
| `max_sessions` | Total number of session channels which can be established across a single SSH connection via Siriusec | |

## Preset roles

Siriusec provides several pre-defined roles out-of-the-box:
...
A Siriusec role can also define which hosts (nodes) a user can have access to.
...
spec:
  ...
  deny:
    node_labels:
      ...
      # Siriusec will deny access to any node labeled as 'database' or 'backup'
...
RBAC lets teams limit what resources are available to Siriusec users. This can be helpful if, for example,
...
```yaml
  allow:
    rules:
    # Role: CRUD options for managing Siriusec Roles
    - resources:
      - role
      verbs: [list, create, read, update, delete]
    ...
    - resources:
      - event
      verbs:  [list, read]
    # User: CRUD options for managing Siriusec local users
    - resources: [user]
      verbs: [list,create,read,update,delete]
