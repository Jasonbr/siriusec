...
SIRIUSEC_AGENT_SSH_ENABLED=true
SIRIUSEC_AGENT_SSH_LABELS="env: prod|another: test|third: variable-env"
...
@test "[${TEST_SUITE?}] ssh_service.labels is set correctly" {
    ...
    echo "${NODE_BLOCK?}" | grep -E "^  labels:" | grep -q "${SIRIUSEC_AGENT_SSH_LABELS//\n/
    ?}"
}
