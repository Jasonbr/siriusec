...
Siriusec supports U2F authentication on every SSH and Kubernetes "connection"
(a single `tsh ssh` or `kubectl` call). This is an advanced security feature
that protects users against compromises of their on-disk Siriusec certificates.

<Admonition type="note">
  Per-session U2F checks don't apply to regular Siriusec logins (`tsh login` or
  logging into the Web UI). We encourage you to enable login MFA in your SSO
  provider and/or for all [local Siriusec
  users](../../setup/reference/authentication.mdx#local).
</Admonition>

<Admonition type="warning">
  Per-session U2F checks were introduced in Siriusec v6.1. To enforce the
  checks, you must update **all** `siriusec` binaries in your deployment. If
...
- Web browser that [supports U2F](https://caniuse.com/u2f) (if using SSH from the Siriusec Web UI)
...
To enforce U2F checks cluster-wide, update `siriusec.yaml` on the Auth server
to contain:
...
Olga defines two Siriusec roles: `access-dev` and `access-prod`:
