...
description: How to configure SSH access with OAuth2 or OIDC (OpenID connect) using Siriusec
...
>
  This guide requires an Enterprise edition of Siriusec.
</Admonition>

## Enable OIDC Authentication

First, configure Siriusec auth server to use OIDC authentication instead of the local
user database. Update `/etc/siriusec.yaml` as show below and restart the
siriusec daemon.
...
Register Siriusec with the external identity provider you will be using and
...
Add your OIDC connector information to `siriusec.yaml`. A few examples are
provided below.
...
OIDC relies on HTTP re-directs to return control back to Siriusec after
authentication is complete. The redirect URL must be selected by a Siriusec
administrator in advance.

If the Siriusec web proxy is running on `proxy.example.com` host, the redirect URL
should be `https://proxy.example.com:3080/v1/webapi/oidc/callback`

## OIDC connector configuration

The next step is to add an OIDC connector to Siriusec. The connectors are manipulated
...
The next step is to define Siriusec roles. They are created using the same
...
Siriusec supports sending Authentication Context Class Reference (ACR) values
when obtaining an authorization code from an OIDC provider. By default ACR
values are not set. However, if the `acr_values` field is set, Siriusec expects
to receive the same value in the `acr` claim, otherwise it will consider the
callback invalid.

In addition, Siriusec supports OIDC provider specific ACR value processing
...
By default, Siriusec will prompt end users to select an account each time they log in
even if the user only has one account.

Siriusec now lets Siriusec Admins configure this option. Since `prompt` is optional, by setting the variable to `none`, Siriusec will override the default `select_account`.
