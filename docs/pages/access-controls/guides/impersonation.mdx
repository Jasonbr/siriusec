---
title: Impersonating Siriusec Users
description: How to issue short-lived certs on behalf of Siriusec users using impersonation.
---

# Impersonation

Sometimes users need to create short-lived certificates for non-interactive
users, for example, CI/CD systems. Your programs interacting with Siriusec may
need to create their own authentication as well. Siriusec's impersonation allows
...
```code
$ tsh login --proxy=siriusec.localhost --user=alice --auth=local
...
$ ssh -J jenkins@siriusec.localhost:3023 -p 3022 jenkins@127.0.0.1
```
...
<Admonition
  ...
  title="Audit"
>
  Siriusec's `session.start` event will capture an action done
...
To prevent unintended consequences, Siriusec defines the following impersonation rules:
...
- Even if the `jenkins` role could impersonate some other roles, Alice would not
  be able to use this permission. Siriusec prevents recursive impersonation.
- Alice can get a new `jenkins` certificate with the same TTL, but metadata
  updated, for example pointing to a different Siriusec leaf cluster. Siriusec
...
$ tsh login --proxy=siriusec.example.com --user=alice --auth=local
