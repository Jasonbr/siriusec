/*
Siriusec is a modern SSH server for remotely accessing clusters
of Linux servers via SSH or HTTPS. It is intended to be used instead of sshd.

Siriusec enables teams to easily adopt the best SSH practices like:

	- No need to distribute keys: Siriusec uses certificate-based access with
	  automatic expiration time.
	- Enforcement of 2nd factor authentication.
	- Cluster introspection: every Siriusec node becomes a part of a cluster
...
Siriusec on Github:
	https://github.com/siriusec/siriusec
*/
package siriusec
