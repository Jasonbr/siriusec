---
title: Access AWS Management Console With Siriusec Application Access
description: How to access AWS Management Console with Siriusec Application Access.
videoBanner: GVcy_rffxQw
---

# AWS Management Console Access

Siriusec can automatically sign your users into AWS management console with
appropriate IAM roles.

This guide will explain how to:

- Connect your AWS account(-s) to Siriusec.
- Setup example AWS IAM Read Only and Power User roles.
- Use Siriusec's role-based access control with AWS IAM roles.
- View Siriusec users' AWS console activity in CloudTrail.

## Prerequisites

- Siriusec with Application Access. Follow [Getting Started](../getting-started.mdx)
  or [Connecting Apps](./connecting-apps.mdx) guides to get it running.
- IAM permissions in the AWS account you want to connect.
- AWS EC2 or other instance where you can assign a IAM Security Role for the Siriusec Agent.

<Admonition type="note">
If using the Siriusec agent deployed in AWS EKS, you cannot use Helm chart
...
Siriusec uses AWS [Federation](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_enable-console-custom-url.html)
...
Do this for each IAM role your Siriusec users will need to assume.

## Step 3. Give Siriusec permissions to assume roles

Next, create a Role using this IAM policy to allow Siriusec to assume IAM roles:
...
Attach this policy to the IAM role/user your Siriusec application service agent
is using.

![AWS Attach Security Role](../../../img/application-access/attach-security-role.png)

## Step 4. Configure Siriusec IAM role mapping

The next step is to give your Siriusec users permissions to assume IAM roles.
...
## Step 5. Register AWS console application in Siriusec
...
siriusec:
  ...
  auth_token: /var/lib/siriusec-app/token
...
app_service:
  ...
  apps:
  - name: "awsconsole"
  # The public AWS Console is used after authenticating the user from Siriusec
...
When showing available IAM roles, Siriusec will display only role ARNs that
belong to the specific account.

## Step 6. Connect to AWS console with assumed IAM role

Navigate to the Applications tab in your Siriusec cluster's control panel and
...
Note that your federated login session is marked with your Siriusec username.

## Step 7. Use CloudTrail to see Siriusec user activity
...
Each Siriusec federated login session uses Siriusec username as the federated
