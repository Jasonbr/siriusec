...
description: Role-Based Access Control (RBAC) for Siriusec Application Access.
---

# Application Access Role-Based Access Control

This article describes Access Control concepts particularly relevant to Siriusec Application Access.

## Assigning labels to applications

Siriusec Application Access uses labels to control access to the proxied
web applications.

Siriusec administrators can assign static and dynamic labels to apps using
configuration:
...
- name: "grafana"
  ...
  labels:
    env: "prod"
    group: "metrics"
  # Siriusec periodically executes commands in dynamic labels and uses the
...
Siriusec administrators can configure roles to allow or deny users' access to
