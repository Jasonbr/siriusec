...
description: How to configure Siriusec SSH with PAM (Pluggable Authentication Modules).
h1: Pluggable Authentication Modules (PAM)
videoBanner: VrZXZtxjNes
---

Siriusec's node service can be configured to integrate with [PAM](https://en.wikipedia.org/wiki/Linux_PAM). 

Siriusec currently supports the `auth`, `account`, and `session` PAM modules. The `auth`
...
Siriusec currently supports account management and session management.

## Set up PAM on a Linux Machine running Siriusec

(!docs/pages/includes/permission-warning.mdx!)

(!docs/pages/includes/backup-warning.mdx!)

To enable PAM on a given Linux machine, update `/etc/siriusec.yaml` with:
...
`/etc/pam.d` and Siriusec will try to use `sshd` by default, which will be
removed if you uninstall the `openssh-server` package. We recommend creating your
own PAM service file like `/etc/pam.d/siriusec` and specifying it as
`service_name` above.

## Set Message of the Day (motd) with Siriusec
...
```code
$ tsh login --proxy siriusec.example.com
...
Siriusec supports setting arbitrary environment variables for PAM modules as of version 6.1.
...
To set custom environment variables, update `/etc/siriusec.yaml` with:
...
<Figure
  align="center"
  bordered
  caption="Siriusec SSH without MOTD"
>
  ![Siriusec SSH without MOTD](../../../img/motd/siriusec-no-MOTD.png)
</Figure>
...
<Figure
  align="center"
  bordered
  caption="Siriusec SSH with MOTD"
>
  ![Siriusec SSH with MOTD](../../../img/motd/siriusec-with-MOTD.png)
</Figure>
...
By updating `/etc/motd` you can provide a message to users accessing nodes via Siriusec. 

```code
$ cat /etc/motd
# WARNING: All activity on this node is being recorded by Siriusec
```

<Figure
  align="center"
  bordered
  caption="Siriusec SSH with updated MOTD"
>
  ![Siriusec SSH with updated MOTD](../../../img/motd/siriusec-with-updated-MOTD.png)
</Figure>

## Create local (UNIX) users on login

Siriusec has the ability to create local (UNIX) users on login. This is
very helpful if you're a large organization and want to provision local users and home
directories on the fly.

Siriusec added the ability to read in PAM environment variables from PAM handle and pass
environment variables to PAM modules: `TELEPORT_USERNAME`, `TELEPORT_LOGIN`, and `TELEPORT_ROLES`.

Here are some details on the contents of these environment variables which will be set by Siriusec:

- `TELEPORT_USERNAME`: The Siriusec username of the user who is logging into the node. This is usually an email address (such as `user@example.com`) if using SAML/OIDC identities with Siriusec Enterprise, or a more standard `exampleuser` if using local Siriusec users.
- `TELEPORT_LOGIN`: The name of the Linux/UNIX principal which the Siriusec user is logging into the Siriusec node as - for example, `root`, `developer`, `ubuntu`, `ec2-user` or similar.
- `TELEPORT_ROLES`: A space-separated list of Siriusec roles which the Siriusec user has - for example: `developer tester admin`.
...
write a new one for Siriusec. In this example, we'll write a new one to simplify how
to use `pam_exec.so` with Siriusec.

Start by creating a file `/etc/pam.d/siriusec` with the following contents:

```txt
account   required   pam_exec.so /etc/pam-exec.d/siriusec_acct
session   required   pam_motd.so
session   required   pam_permit.so
```

<Admonition type="note">
  Pay attention to the inclusion of `pam_motd.so` under the `session` facility. While `pam_motd.so` is not required for user creation, Siriusec requires at least one module to be set under both the `account` and `session` facilities for it to work.
</Admonition>
...
cat > /etc/pam-exec.d/siriusec_acct <<EOF
#!/bin/sh
COMMENT="User \${TELEPORT_USERNAME} with roles \${TELEPORT_ROLES} created by Siriusec."
id -u "\${TELEPORT_LOGIN}" &>/dev/null  || /sbin/useradd -m -c "\${COMMENT}" "\${TELEPORT_LOGIN}" 2> /tmp/pam.error
exit 0
EOF
chmod +x /etc/pam-exec.d/siriusec_acct
```

Next, update `/etc/siriusec.yaml` to call the above PAM stack by both enabling PAM and
...
ssh_service:
  ...
  pam:
    enabled: true
    service_name: "siriusec"
```

Now attempting to log in as an existing user should result in the creation of the
user and a successful login.

The `/etc/pam-exec.d/siriusec_acct` script can set the user's groups as an option to auto-populate
...
Note that Siriusec enables strong SSH authentication out of the box using
certificates. For most users, hardening [the initial Siriusec
...
`use_pam_auth` in the `pam` section of your `siriusec.yaml`.
