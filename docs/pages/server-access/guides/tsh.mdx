...
This User Manual covers usage of the Siriusec client tool, `tsh` and Siriusec's Web interface.
...
```code
# Login into a Siriusec cluster. This command retrieves user's certificates
# and saves them into ~/.tsh/siriusec.example.com
...
# ... and now your 'ssh' command is calling Siriusec's `tsh ssh`
$ ssh user@host
...
In other words, Siriusec was designed to be fully compatible with existing
...
A user identity in Siriusec exists in the scope of a cluster. The member nodes
of a cluster may have multiple OS users on them. A Siriusec administrator
assigns allowed logins to every Siriusec user account.

When logging into a remote node, you will have to specify both logins. Siriusec
...
This allows you to authenticate just once, maybe at the beginning of the day. Subsequent `tsh ssh` commands will run without asking for credentials until the temporary certificate expires. By default, Siriusec issues user certificates with a time to live (TTL) of 12 hours.
...
A Siriusec cluster can be configured for multiple user identity sources. For example, a cluster may have a local user called `admin` while regular users should [authenticate via Github](../../setup/admin/github-sso.mdx). In this case, you have to pass `--auth` flag to `tsh login` to specify which identity storage to use:

```code
# Login using the local Siriusec 'admin' user:
...
Regular users of Siriusec must request an auto-expiring SSH certificate, usually
every day. This doesn't work for non-interactive scripts, like cron jobs or
CI/CD pipeline.

For such automation, it is recommended to create a separate Siriusec user for
...
```code
# To be executed on a Siriusec auth server
...
In a Siriusec cluster, all nodes periodically ping the cluster's auth server and
update their status. This allows Siriusec users to see which nodes are online
...
A Siriusec proxy uses two ports: `3080` for HTTPS and `3023` for proxying SSH
connections. The HTTPS port is used to serve Web UI and also to implement 2nd
factor auth for the `tsh` client.

If a Siriusec proxy is configured to listen on non-default ports, they must be
...
While implementing ProxyJump for Siriusec, we have extended the feature to `tsh`.
...
- Only one jump host is supported (`-J` supports chaining that Siriusec does not utilize) and `tsh` will return with error in the case of two jumphosts, i.e. `-J proxy-1.example.com,proxy-2.example.com` will not work.
- When `tsh ssh -J user@proxy` is used, it overrides the SSH proxy defined in the tsh profile, and port forwarding is used instead of the existing Siriusec proxy subsystem.
...
2. **Nodename setting**: siriusec daemon supports the` nodename` flag, which allows Siriusec administrators to assign alternative node names.
...
The default TTL of a Siriusec user certificate is 12 hours. This can be modified
...
Siriusec makes this more convenient. Let's log into a server named `luna`
and ask Siriusec for our current session status:
...
Siriusec supports creating clusters of servers located behind firewalls
**without any open listening TCP ports**.  This works by creating reverse SSH
tunnels from behind-firewall environments into a Siriusec proxy you have access to.
...
Assuming the `work` Siriusec proxy server is configured with a few trusted
