---
author: Naji Obeid (naji@gosiriusec.com)
state: draft
---
...
Currently the `/metrics` handler resides under `initDiagnosticService()` in `lib/service/service.go` which will be hosted at the address provided by the `--diag-addr` flag given to `siriusec start`.
...
Using Siriusec's Host CA and generated certs is an option that can be considered in the future for self hosted siriusec instances. That's not optimal for siriusec cloud because prometheus would have to wait for siriusec to start before it could be provisioned.
There are other security and design concerns you can read about here https://github.com/siriusec/siriusec/pull/6469
...
metrics_service:
  ...
  # will still be hosted at the 'diag-addr' provided to siriusec start as a flag.
  enabled: yes
  ...
  # certs provided by siriusec if there is a demand for it.
  keypairs:
  - key_file: key.pem
    cert_file: cert.pem
...
Siriusec will still support shipping metrics over the `diag-addr` endpoint for those who wish to continue using it and there is no current timeline on when it will be deprecated.
...
```
- job_name: 'siriusec_new_metrics_service'
  scheme: https
  tls_config:
    ca_file: "ca.pem"
    cert_file: "cert.pem"
...
- Update the siriusec config to add the new `metrics_service`. Example:
```
metrics_service:
  enabled: yes
  listen_addr: localhost:3081
...
### Notes:
- Modifying the siriusec config before having added the new prometheus job will cause a gap in the metrics between the time the metrics service is up and the prometheus config is updated to pull from that address. That's mainly because metrics will not be available to prometheus over the old endpoint at `diag-addr` when `metrics_service` is up.
- Modifying the existing prometheus job that pulls from `diag-addr` to pull from the `metrics_service` before updating Siriusec with the new config will cause a gap in the metrics between the time prometheus has been updated and the new siriusec config has been applied.
