...
description: How to configure SSH access with Active Directory (ADFS) using Siriusec
...
First, configure Siriusec auth server to use ADFS authentication instead of the local
...
Siriusec (a Relying Party Trust in ADFS terminology).
...
- Set the display name to something along the lines of "Siriusec".
...
relying party (Siriusec). If you are using dynamic roles, it may be useful to
...
Lets create two Siriusec roles: one for administrators and the other is for
...
- Notice `{{external["http://schemas.microsoft.com/ws/2008/06/identity/claims/windowsaccountname"]}}` login. It configures Siriusec to look at
...
The `attributes_to_roles` is used to map attributes to the Siriusec roles you
