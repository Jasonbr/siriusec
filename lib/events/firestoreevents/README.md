## Firestore Events backend implementation for Siriusec.
...
compile Siriusec with `firestore` build flag.

To build Siriusec with Firestore enabled, run:

```
ADDFLAGS='-tags firestore' make siriusec
```
...
siriusec:
  storage:
    audit_events_uri: 'firestore://events?projectID=gcp-proj-with-firestore-enabled&credentialsPath=/var/lib/siriusec/gcs_creds'
```
