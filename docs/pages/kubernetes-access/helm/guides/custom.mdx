---
title: Running a Siriusec cluster with a custom configuration using Helm
description: Install and configure a Siriusec cluster with a custom configuration using Helm
---

In this guide, we'll go through how to set up a Siriusec cluster in Kubernetes using a custom [`siriusec.yaml`](../../../setup/reference/config.mdx)
config file using Siriusec Helm charts.

This setup can be useful when you already have an existing Siriusec cluster and would like to start running it in Kubernetes, or when
migrating your setup from a legacy version of the Helm charts.

(!docs/pages/kubernetes-access/helm/includes/helm-install.mdx!)

## Step 3. Setting up a Siriusec cluster with Helm using a custom config

In `custom` mode, the `siriusec-cluster` Helm chart does not create a `ConfigMap` containing a `siriusec.yaml` file for you, but
expects that you will provide this yourself.

For this example, we'll be using this `siriusec.yaml` configuration file (with appropriately complex [static tokens](../../../setup/admin/adding-nodes.mdx#insecure-static-tokens)):

```bash
$ cat << EOF > siriusec.yaml
siriusec:
  log:
    output: stderr
    severity: INFO
...
<Admonition type="tip">
  You can skip this step if you already have a `siriusec.yaml` file locally that you'd like to use.
</Admonition>

You can create the namespace for the config and add the `siriusec.yaml` from your local disk like this:

```code
$ kubectl create namespace siriusec
$ kubectl --namespace siriusec create configmap siriusec --from-file=siriusec.yaml
```
...
<Admonition type="note">
  ...
  In this example, it's `siriusec`.

  The name (key) of the configuration file uploaded to your `ConfigMap` must be `siriusec.yaml`. If your configuration file is named differently
  on disk, you can specify the key that should be used in the `kubectl` command:

  ```code
  $ kubectl --namespace siriusec create configmap siriusec --from-file=siriusec.yaml=my-siriusec-config-file.yaml
  ```
</Admonition>
...
<Admonition type="warning">
  ...
  It's important to specify any settings under the `acme`, `aws`, `gcp`, and `logLevel` sections of the chart in your own `siriusec.yaml` file that you upload yourself.
</Admonition>

You can control the externally-facing name of your cluster using the `public_addr` sections of `siriusec.yaml`. In this example,
...
<Admonition type="tip">
  ...
  Don't worry if you can't - you'll just have to remember to replace `custom.example.com` with the external IP or hostname of the Kubernetes load balancer to be able to access Siriusec from your local machine.
</Admonition>
...
# pod/siriusec-5c56b4d869-znmqk   1/1     Running   0          5h8m

# NAME               TYPE           CLUSTER-IP       EXTERNAL-IP                                                               PORT(S)                                                      AGE
# service/siriusec   LoadBalancer   10.100.162.158   a5f22a02798f541e58c6641c1b158ea3-1989279894.us-east-1.elb.amazonaws.com   443:30945/TCP,3023:32342/TCP,3026:30851/TCP,3024:31521/TCP   5h29m

# NAME                       READY   UP-TO-DATE   AVAILABLE   AGE
# deployment.apps/siriusec   1/1     1            1           5h29m

# NAME                                  DESIRED   CURRENT   READY   AGE
# replicaset.apps/siriusec-5c56b4d869   1         1         1       5h8m
```

## Step 4. Create a Siriusec user (optional)

If you're not migrating an existing Siriusec cluster, you'll need to create a user to be able to log into Siriusec. This needs to be done on the
Siriusec auth server, so we can run the command using `kubectl`:
...
# NOTE: Make sure custom.example.com:443 points at a Siriusec proxy that users can access.
```
...
<Admonition type="warning">
  Using a Kubernetes-issued load balancer IP or hostname is OK for testing but is not viable when running a production Siriusec cluster
...
### Making changes to `siriusec.yaml`

If you make changes to your Siriusec `ConfigMap`, you can apply these changes by deleting the old `ConfigMap` and applying a new one:

```code
$ kubectl --namespace siriusec delete configmap siriusec && \
# kubectl --namespace siriusec create configmap siriusec --from-file=siriusec.yaml
```

<Admonition type="note">
  Make sure that the name of the `ConfigMap` (e.g. `siriusec`) matches the Helm release name used as described above.

  You can list all available `ConfigMap`s in your namespace using this command:

  ```code
  $ kubectl --namespace siriusec get configmap
...
After editing the `ConfigMap`, you must initiate a rolling restart of your Siriusec deployment to pick up the changed `ConfigMap`:

```code
$ kubectl --namespace siriusec rollout restart deploy/siriusec
```

### Making changes to other Helm values

To make changes to your Siriusec cluster after deployment which are not covered by the functionality in `siriusec.yaml`, you can
use `helm upgrade`.
