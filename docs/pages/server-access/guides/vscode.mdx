...
description: How to use Visual Studio Code's Remote Development plugin with Siriusec
h1: Remote Development With Visual Studio Code
---

This guide explains how to use Siriusec and Visual Studio Code's remote SSH extension.

## Prerequisites

- [tsh client tool](https://gosiriusec.com/siriusec/download) >= (=siriusec.version=).
- OpenSSH client.
...
Configure your local SSH client to access Siriusec nodes:
...
<Admonition type="note">
Siriusec's certificates expire fairly quickly, after which SSH
...
![Connecting to a Siriusec host in VS Code](../../../img/vscode/connect.png)
</Figure>
...
![VS Code connected to a Siriusec node](../../../img/vscode/connected-editor.png)
</Figure>
...
It's possible to remotely develop on any OpenSSH host joined to a Siriusec
...
Siriusec certificates, after which the procedure outlined above can be used to
connect to the host in VS Code.

### Using OpenSSH clients

This guide makes use of `tsh config`, added in Siriusec 7.0; refer to the
...
to use the VS Code's remote SSH extension with older Siriusec clients.
