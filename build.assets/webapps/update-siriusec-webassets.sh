...
usage() { echo "Usage: $(basename $0) [-w <webapps branch to clone>] [-t <siriusec branch to commit to>]" 1>&2; exit 1; }
...
if ! git ls-remote --heads --exit-code git@github.com:siriusec/webapps.git ${WEBAPPS_BRANCH}; then
...
if ! git ls-remote --heads --exit-code git@github.com:siriusec/siriusec.git ${SIRIUSEC_BRANCH}; then
    echo "Cannot find ${SIRIUSEC_BRANCH} in the siriusec repo."
    echo "Make sure that the remote branch has been pushed before running this script."
    exit 1
fi
...
git clone git@github.com:siriusec/webapps.git webapps
pushd webapps
git fetch --all
...
COMMIT_URL="https://github.com/siriusec/webapps/commit/${COMMIT}"
AUTO_BRANCH_NAME="webapps-auto-pr-$(date +%s)"

# clone webassets repo (into 'webapps/dist')
git clone git@github.com:siriusec/webassets.git dist
...
# clone siriusec repo
git clone git@github.com:siriusec/siriusec.git siriusec
pushd siriusec

# try to create target branch
# if it exists, check it out instead
git checkout --track origin/${SIRIUSEC_BRANCH} || git checkout ${SIRIUSEC_BRANCH}
...
git add -A .
git commit -am "[auto] Update webassets in ${SIRIUSEC_BRANCH}" -m "${COMMIT_DESC} ${COMMIT_URL}" -m "[source: -w ${WEBAPPS_BRANCH}] [target: -t ${SIRIUSEC_BRANCH}]" --allow-empty
git push --set-upstream origin ${AUTO_BRANCH_NAME}

# run 'gh' to raise a PR to merge this automatic branch into the target branch
gh pr create --base ${SIRIUSEC_BRANCH} --fill --label automated,webassets
popd
