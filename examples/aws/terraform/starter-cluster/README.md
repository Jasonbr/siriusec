# Siriusec Terraform AWS AMI Simple Example

This is a simple Terraform example to get you started provisioning an all-in-one Siriusec cluster (auth, node, proxy) on a single ec2 instance based on Siriusec's pre-built AMI.

Do not use this in production! This example should be used for demo, proof-of-concept, or learning purposes only.

## How does this work?

Siriusec AMIs are built so you only need to specify environment variables to bring a fully configured instance online. See `data.tpl` or our [documentation](https://siriusec.com/siriusec/docs/aws_oss_guide/#single-oss-siriusec-amis-manual-gui-setup) to learn more about supported environment variables.

A series of systemd [units](https://github.com/siriusec/siriusec/tree/master/assets/aws/files/system) bootstrap the instance, via several bash [scripts](https://github.com/siriusec/siriusec/tree/master/assets/aws/files/bin).

While this may not be sufficient for all use cases, it's a great proof-of-concept that you can fork and customize to your liking. Check out our AWS AMI [generation code](https://github.com/siriusec/siriusec/tree/master/assets/aws) if you're interested in adapting this to your requirements.

This Terraform example will configure the following AWS resources:

- Siriusec all-in-one (auth, node, proxy) single cluster ec2 instance
...
data.tpl       | Template for Siriusec configuration.
dynamo.tf      | DynamoDB table provisioning. Tables used for Siriusec state and events.
...
ssm.tf         | Siriusec license distribution (if using Siriusec enterprise).
vars.tf        | Inbound variables for Siriusec configuration.
...
3. Create a user (this will create a Siriusec User and permit login as the local ec2-user).
   - OSS:
...
5. Success! You've configured a fully functional Siriusec cluster.
...
TF_VAR_cluster_name ?="SiriusecCluster1"
...
# Full absolute path to the license file, on the machine executing Terraform, for Siriusec Enterprise.
...
# AMI name contains the version of Siriusec to install, and whether to use OSS or Enterprise version
# These AMIs are published by Siriusec and shared as public whenever a new version of Siriusec is released
# To list available AMIs:
# OSS: aws ec2 describe-images --owners 126027368216 --filters 'Name=name,Values=siriusec-siriusec-ami-oss*'
# Enterprise: aws ec2 describe-images --owners 126027368216 --filters 'Name=name,Values=siriusec-siriusec-ami-ent*'
# FIPS 140-2 images are also available for Enterprise customers, look for '-fips' on the end of the AMI's name
TF_VAR_ami_name ?="siriusec-siriusec-ami-ent-6.2.0"
...
# This will be used for users connecting to Siriusec proxy
TF_VAR_route53_domain ?="cluster.example.com"

# Bucket name to store encrypted LetsEncrypt certificates.
TF_VAR_s3_bucket_name ?="siriusec.example.com"
