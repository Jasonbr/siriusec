...
description: How to configure Siriusec Database Access with AWS RDS/Aurora MySQL.
...
Siriusec Database Access for AWS RDS and Aurora uses IAM authentication which
...
To allow Siriusec database service to log into the database instance using auth
...
Siriusec Database Access for MySQL on AWS is available starting from `6.0`
release.
...
```bash
$ tctl --config=/path/to/siriusec.yaml create <<EOF
kind: role
version: v4
metadata:
  name: db
...
```code
$ tctl --config=/path/to/siriusec.yaml users add --roles=admin,db testuser
```

### Start Database service with CLI flags

For a quick try-out, Siriusec database service doesn't require a configuration
...
Note that the `--auth-server` flag must point to the Siriusec cluster's proxy endpoint
...
siriusec:
  ...
  # machine as Siriusec auth and proxy.
  data_dir: /var/lib/siriusec-db
  nodename: siriusec-db-instance
  # Siriusec invitation token used to join a cluster.
...
When setting up Siriusec database service with AWS RDS or Aurora, it must have
...
Siriusec database service uses the default credential provider chain to find AWS
...
```code
$ tsh login --proxy=siriusec.example.com --user=testuser
$ tsh db ls
