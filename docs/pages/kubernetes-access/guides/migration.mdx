...
description: How to migrate pre-5.0 Siriusec clusters to new configuration.
h1: Migrating to Siriusec Kubernetes Access 5.0
---

In release 5.0, Siriusec has changed the [Kubernetes integration](../../kubernetes-access/introduction.mdx) to improve configuration and user experience.
These changes are backward compatible and there is no *required* migration.
However, to get the most out of Siriusec, you should consider migrating as
described below.

The main changes in 5.0 are:

- New `kubernetes_service` configuration section in `siriusec.yaml`, decoupled
  from `proxy_service`
- Support for multiple Kubernetes clusters per Siriusec cluster (replaces the
...
still required. A proxy is always the public-facing gateway into the Siriusec
cluster, acting as an authentication point and a connection router.

## Example scenarios

We'll describe the migration for several common Siriusec configurations.

### Standalone proxy for a single cluster

In this scenario, we're running a Siriusec proxy outside of the Kubernetes
...
In this scenario, we're running a Siriusec proxy inside of the Kubernetes
...
In this scenario, we're running multiple Trusted Siriusec clusters, one for
each Kubernetes cluster.

#### Recommend Option 1: Siriusec Kubernetes agents in each Kubernetes cluster

This is the recommended approach for handling multiple Kubernetes clusters.

Instead of running multiple Siriusec clusters (auth and proxy for each
Kubernetes cluster), we will connect multiple `kubernetes_service` agents to
the same root Siriusec cluster.

Each `kubernetes_service` will create its own reverse tunnel back to a Siriusec
...
After all the Siriusec agents are running, you can view the list of Kubernetes
...
#### Option 2: One Siriusec agent connected to multiple clusters
...
Trusted Clusters. It only shows the Kubernetes clusters from the Siriusec
...
subset of Kubernetes clusters using Kubernetes cluster labels and Siriusec
roles.
