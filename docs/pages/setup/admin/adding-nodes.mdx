...
This guide explains how to add nodes to Open Source, Enterprise Siriusec,
self-hosted or cloud editions.

## Prerequisites

- Installed [Siriusec](../../getting-started.mdx) or [Siriusec Cloud](../../cloud/introduction.mdx) >= (=siriusec.version=)
- [Tctl admin tool](https://gosiriusec.com/siriusec/download) >= (=siriusec.version=)

(!docs/pages/includes/tctl.mdx!)

<Admonition type="note">
For cloud, login with a siriusec user with editor privileges:
```code
...
Siriusec only allows access to nodes that have joined the cluster.

Once a node joins, it receives its own host certificate signed by the cluster's auth server.

To receive a host certificate upon joining a cluster, a new Siriusec host must present an *invite token*.
...
$ sudo siriusec start --roles=node --token=(=presets.tokens.first=) --auth-server=10.0.10.5

# Adding a new regular SSH node using Siriusec Node Tunneling:
$ sudo siriusec start --roles=node --token=(=presets.tokens.first=) --auth-server=siriusec-proxy.example.com:3080

# Adding a new proxy service on the cluster:
$ sudo siriusec start --roles=proxy --token=(=presets.tokens.first=) --auth-server=10.0.10.5
```
...
As you have seen above, Siriusec uses tokens to invite users to a cluster
...
<Admonition type="note">
  This feature is sometimes called "Siriusec IoT" or node tunneling.
</Admonition>
...
Siriusec Node Tunneling lets you add a remote node to an existing Siriusec Cluster through a tunnel.
...
replaced the auth server IP with the proxy web endpoint `siriusec-proxy.example.com:3080`.
...
$ sudo siriusec start --roles=node --token=/path/to/token.file --auth-server=siriusec-proxy.example.com:3080
```
...
`web_listen_addr` respectively within the `proxy_service`.  Siriusec will automatically recognize using the same port and enable multiplexing. If the log setting is set to DEBUG you will see multiplexing enabled in the server log.
...
>
  ...
  uses a round-robin or a similar balancing algorithm. Do not use sticky load balancing algorithms (a.k.a. "session affinity") with Siriusec proxies.
</Admonition>
...
<Admonition
  ...
  title="Warning"
>
  If a CA pin is not provided, the Siriusec Node will join a
...
<Admonition
  ...
  title="Warning"
>
  The CA pin becomes invalid if a Siriusec administrator
...
```yaml
# Config section in `/etc/siriusec.yaml` file for the auth server
auth_service:
    enabled: true
    tokens:
