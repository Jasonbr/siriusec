...
description: How to configure Siriusec Database Access with self-hosted MongoDB.
...
1. Install and configure Siriusec for Database Access.
2. Configure mutual TLS authentication between Siriusec and your MongoDB cluster.
3. Connect to your MongoDB instance via Siriusec.

## Prerequisites

- Siriusec version `7.0` or newer.
- MongoDB cluster (standalone or replica set) version `(=mongodb.min_version=)` or newer.

<Admonition type="note">
  Siriusec Database Access supports MongoDB `(=mongodb.min_version=)` and newer.
...
## Step 1/3. Install & configure Siriusec
...
Start Siriusec Database service:
...
<Admonition type="note">
The `--auth-server` flag must point to the Siriusec cluster's proxy endpoint
...
By default, Siriusec will connect to the primary replica set member. If you'd
like to connect to a secondary instead, Siriusec will respect `readPreference`
...
Siriusec will use [X.509 authentication](https://docs.mongodb.com/manual/tutorial/configure-x509-client-authentication/)
...
When connecting to a MongoDB server, say as a user `alice`, Siriusec will sign
...
<Tabs>
  ...
  which Siriusec will be connecting to it.
  ...
  The command will create 2 files: `mongo.cas` with Siriusec's certificate
  ...
  Each command will create 2 files: `mongo1.cas`/`mongo2.cas` with Siriusec's
...
Log into your Siriusec cluster and see available databases:
