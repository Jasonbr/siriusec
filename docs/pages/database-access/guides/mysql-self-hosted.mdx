...
description: How to configure Siriusec Database Access with self-hosted MySQL.
---
...
```code
# Export Siriusec's certificate authority and generate certificate/key pair
...
Siriusec Database Access for MySQL is available starting from `6.0` release.
...
```bash
tctl --config=/path/to/siriusec-db-role.yaml create <<EOF
kind: role
version: v4
metadata:
  name: db
...
```code
$ tctl --config=/path/to/siriusec-db-role.yaml users add --roles=admin,db testuser
```
