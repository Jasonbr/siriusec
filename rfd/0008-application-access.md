---
authors: Russell Jones (rjones@gosiriusec.com)
state: implemented
---

# RFD 8 - Siriusec Application Access

## What

This document contains technical implementation details of Siriusec Application Access.

## Why

## Use Cases

The initial implementation of Siriusec Application Access is targeted at users that would like to expose internal applications and dashboards on the public internet.

## Details

### Identity Headers

As described in a previous section, Siriusec uses TLS mutual authentication to pass identity information between internal components. However, identity information is passed to proxied applications in the form of a signed JWT in a request header named `siriusec-jwt-assertion`.

This identity information can be used to show the identity of the user currently logged in, and also change the state of the internal application. For example, because Siriusec roles are forwarded to proxied applications within the JWT header, an control panel application could show either a regular or admin view based on the Siriusec identity of the user.

#### Issuance

All Siriusec clusters have a User and Host CA used to issue user and host SSH and TLS certificates. Siriusec Application Access introduces a JWT signer to each cluster to issue JWTs. The JWT signer uses 2048-bit RSA keys similar to the existing CAs.
...
We've provided a small program available at [examples/jwt/verify-jwt.go](https://github.com/siriusec/siriusec/blob/master/examples/jwt/verify-jwt.go) that can be used to verify a JWT. When you run it, you should see output like the following:

```
# Replace "..." with the JWT when running the program below.
#
# To validate claims, provide the expected claims of issuer (cluster name),
# subject (Siriusec username), and audience (URI of internal application).
...
* `iss`: Issuer of the JWT. This value is the name of the Siriusec cluster issuing the token.
* `nbf`: "Not before" time of the JWT. This is the time at which the JWT becomes valid.
* `sub`: Subject of the JWT. This is the Siriusec identity of the user to whom the JWT was issued.

The following private claims are included.

* `username`: Similar to sub. This is the Siriusec identity of the user to whom the JWT was issued.
* `roles`: List of Siriusec roles assigned to the user.
...
To explicitly log a session out, an authenticated session can issue a `GET /siriusec-logout` or a `DELETE /siriusec-logout` request.

Internal applications and implementers are encouraged to support `DELETE /siriusec-logout` in the form of a logout button within the internal application.

The `GET /siriusec-logout` endpoint is for internal applications that can not be modified. For example, you may go to `https://grafana.proxy.example.com/siriusec-logout` to log out of the ACME application.
...
Siriusec server configuration has been updated to add file, process, and role configuration.
...
proxy_service:
  ...
  https_keypairs:
    - key_file: /var/lib/siriusec/certs/proxy.example.com+3-key.pem
      cert_file: /var/lib/siriusec/certs/proxy.example.com+3.pem
    - key_file: /var/lib/siriusec/certs/app.example.com+3-key.pem
      cert_file: /var/lib/siriusec/certs/app.example.com+3.pem
```
...
app_service:
   ...
   - name: jenkins
     ...
     # is avaiable at.
     public_addr: jenkins.acme.siriusec.dev
...
The `siriusec` process has been updated to support three additional flags that can be used to configure an application service in addition to the existing `--roles` and `--labels` flag (which now apply to the application, not the service).
...
$ siriusec start \
   ...
   --app-public-addr="jenkins.acme.siriusec.dev"
```
...
Siriusec client configuration has been updated to add subcommands to both `tctl` and `tsh`.
...
Please note:
  ...
  - Update DNS to point example-app.proxy.example.com:3080 to the Siriusec proxy.
  - Add a TLS certificate for example-app.proxy.example.com:3080 to the Siriusec proxy under "https_keypairs".
```
