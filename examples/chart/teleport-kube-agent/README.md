# Siriusec Agent chart

This chart is a Siriusec agent used to register any or all of the following services
with an existing Siriusec cluster:
- Siriusec Kubernetes access
- Siriusec Application access
- Siriusec Database access

To use it, you will need:
- an existing Siriusec cluster (at least proxy and auth services)
- a reachable proxy endpoint (`$PROXY_ENDPOINT` e.g. `siriusec.example.com:3080` or `siriusec.example.com:443`) 
- a reachable reverse tunnel port on the proxy (e.g. `siriusec.example.com:3024`). The address is automatically 
  retrieved from the Siriusec proxy configuration.
- either a static or dynamic join token for the Siriusec Cluster
  - a [static join token](https://gosiriusec.com/siriusec/docs/admin-guide/#adding-nodes-to-the-cluster)
    for this Siriusec cluster (`$JOIN_TOKEN`) is used by default.
  - optionally a [dynamic join token](https://gosiriusec.com/siriusec/docs/admin-guide/#adding-nodes-to-the-cluster) can
...
To provide backwards compatibility with older versions of the `siriusec-kube-agent` chart, if you do
not specify any value for `roles`, the chart will run with only the `kube` role enabled.

## Kubernetes access

To use Siriusec Kubernetes access, you will also need:
...
```sh
$ helm install siriusec-kube-agent . \
  --create-namespace \
  --namespace siriusec \
  --set roles=kube \
...
export TELEPORT_NAMESPACE=siriusec
...
kind: Secret
metadata:
  name: siriusec-kube-agent-join-token
...
Note that due to backwards compatbility, the `labels` value **only** applies to the Siriusec
Kubernetes service. To set labels for applications or databases, use the different formats
detailed below.

## Application access

To use Siriusec Application access, you will also need:
...
| `public_addr` | Public address used to access the app | `apps[0].public_addr=grafana.siriusec.example.com` | | No |
...
To use Siriusec database access, you will also need:
...
```sh
$ kubectl logs -n siriusec deployment/siriusec-kube-agent
```
