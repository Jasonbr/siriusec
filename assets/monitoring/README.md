...
Siriusec exports Prometheus Metrics endpoints.

**Start Siriusec With Prometheus Endpoint**

To start siriusec with prometheus endpoint enabled:

```bash
siriusec start --diag-addr=127.0.0.1:3000
...
Siriusec can be started with Go's standard profiler [pprof](https://golang.org/pkg/net/http/pprof/).

```bash
$ siriusec start -d --diag-addr=127.0.0.1:3000
# http://127.0.0.1:3000/debug/pprof/
```

When siriusec is started in debug mode (with siriusec start -d flag) Go’s CPU,
