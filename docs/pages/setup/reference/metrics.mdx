---
title: Siriusec Metrics
description: How to set up Prometheus to monitor Siriusec for SSH and Kubernetes access
h1: Metrics
---

## Siriusec Prometheus endpoint

Siriusec provides HTTP endpoints for monitoring purposes. They are disabled
by default, but you can enable them using the `--diag-addr` flag to `siriusec start`:

```code
$ sudo siriusec start --diag-addr=127.0.0.1:3000
```

Now you can see the monitoring information by visiting several endpoints:

- `http://127.0.0.1:3000/metrics` is the list of internal metrics Siriusec is
...
| - | - | - | - |
| `audit_failed_disk_monitoring` | counter | Siriusec Audit Log | Number of times disk monitoring failed. |
| `audit_failed_emit_events` | counter | Siriusec Audit Log | Number of times emitting audit event failed. |
| `audit_percentage_disk_space_used` | gauge | Siriusec Audit Log | Percentage disk space used. |
| `audit_server_open_files` | gauge | Siriusec Audit Log | Number of open audit files. |
| `auth_generate_requests` | gauge | Siriusec Auth | Number of current generate requests. |
| `auth_generate_requests_throttled_total` | counter | Siriusec Auth | Number of throttled requests to generate new server keys. |
| `auth_generate_requests_total` | counter | Siriusec Auth | Number of requests to generate new server keys. |
| `auth_generate_seconds` | `histogram` | Siriusec Auth | Latency for generate requests. |
...
| `certificate_mismatch_total` | counter | Siriusec Proxy | Number of times there was a certificate mismatch. |
| `cluster_name_not_found_total` | counter | Siriusec Auth | Number of times a cluster was not found. |
...
| `failed_connect_to_node_attempts_total` | counter | Siriusec Proxy | Number of times a user failed connecting to a node |
| `failed_login_attempts_total` | counter | Siriusec Proxy | Number of failed `tsh login` or `tsh ssh` logins. |
...
| `heartbeat_connections_received_total` | counter | Siriusec Auth | Number of times auth received a heartbeat connection. |
| `heartbeat_connections_missed_total` | counter | Siriusec Auth | Number of times auth did not receive a heartbeat from a node. |
...
| `proxy_connection_limit_exceeded_total` | counter | Siriusec Proxy | Number of connections that exceeded the proxy connection limit. |
| `reversetunnel_connected_proxies` | gauge | Siriusec | Number of known proxies being sought. |
| `rx` | counter | Siriusec | Number of bytes received. |
| `server_interactive_sessions_total` | gauge | Siriusec | Number of active sessions. |
| `trusted_clusters` | gauge | Siriusec | Number of tunnels per state. |
| `tx` | counter | Siriusec | Number of bytes transmitted. |
| `user_login_total` | counter | Siriusec Auth | Number of user logins. |
| `user_max_concurrent_sessions_hit_total` | counter | Siriusec Node | Number of times a user exceeded their concurrent session limit. |
