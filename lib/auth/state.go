...
func (p *ProcessStorage) GetState(role types.SystemRole) (*StateV2, error) {
	item, err := p.Get(context.Background(), backend.Key(statesPrefix, strings.ToLower(role.String()), stateName))
	if err != nil {
...
func (p *ProcessStorage) CreateState(role types.SystemRole, state StateV2) error {
	...
	_, err = p.Create(context.Background(), item)
	if err != nil {
		return trace.Wrap(err)
	}
	return nil
...
func (p *ProcessStorage) WriteState(role types.SystemRole, state StateV2) error {
	...
	_, err = p.Put(context.Background(), item)
	if err != nil {
		return trace.Wrap(err)
	}
	return nil
...
func (p *ProcessStorage) ReadIdentity(name string, role types.SystemRole) (*Identity, error) {
	...
	item, err := p.Get(context.Background(), backend.Key(idsPrefix, strings.ToLower(role.String()), name))
	if err != nil {
		return nil, trace.Wrap(err)
	}
	var res IdentityV2
