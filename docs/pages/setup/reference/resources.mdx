---
title: Siriusec Resources Reference
description: The detailed reference documentation for Siriusec Configuration Resources
h1: Siriusec Configuration Resources Reference
---

## Resources

A Siriusec administrator has two tools to configure a Siriusec cluster:
...
Every resource in Siriusec has three required fields:

- `Kind` - The type of resource
- `Name` - A required field in the `metadata` to uniquely identify the resource
- `Version` - The version of the resource format

Everything else is resource-specific and any component of a Siriusec cluster can
...
<Admonition
  ...
  title="YAML Format"
>
  By default Siriusec uses [YAML format](https://en.wikipedia.org/wiki/YAML) to describe resources.
...
spec:
  roles: admin
  status:
    # Users can be temporarily locked in a Siriusec system, but this
  ...
  traits:
    ...
    - joe
    - root
  # Any resource in Siriusec can automatically expire.
...
| - | - |
| [user](#user) | A user record in the internal Siriusec user DB. |
...
Siriusec supports interactive local users, non-interactive local users (bots)
...
spec:
  ...
  roles:
  - admin
  # status sets user temporarily locked in a Siriusec system, for example
...
spec:
  ...
  # conflicting options, siriusec chooses the least permissive value.
  options:
    ...
    # cluster. This setting slows down Siriusec performance because it has to track
  ...
  allow:
    ...
    node_labels:
      ...
      # Siriusec uses golang regexp syntax.
