...
technical Siriusec changes. It's also a process by which these documents are
proposed, discussed, approved and tracked.

## Why

As the Siriusec project grows, we need a way to discuss major changes (e.g. new
features, major refactors, major distribution changes).

Prior to RFDs, Siriusec engineers used several other discussion methods (Google
...
- discussions are in the open, for Siriusec users to see and contribute
...
https://github.com/siriusec/siriusec/tree/master/rfd and has a unique
number.
...
in the main Siriusec branch.

1. pick the RFD number

   check [submitted](https://github.com/siriusec/siriusec/tree/master/rfd)
   and
   [pending](https://github.com/siriusec/siriusec/pulls?q=is%3Apr+is%3Aopen+label%3Arfd)
   RFDs and pick the next available number.

   For example, you're writing an RFD titled 'Siriusec IRC Access' and end up
   with number 18.
...
time. `implemented` is relevant to Siriusec users, but `draft` is mostly for
Siriusec engineers and early adopters.
