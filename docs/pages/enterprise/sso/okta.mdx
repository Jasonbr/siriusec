...
First, configure Siriusec auth server to use SAML authentication instead of the local
...
- Single sign on URL `https://siriusec-proxy.example.com:443/v1/webapi/saml/acs`
- Audience URI (SP Entity ID)`https://siriusec-proxy.example.com:443/v1/webapi/saml/acs`
...
configure a Siriusec connector:
...
## Create a Developer Siriusec Role

We are going to create a new role that'll pull in external information from Okta. Notice 
`{{external.username}}` login. It configures Siriusec to look at *"username"* Okta claim 
