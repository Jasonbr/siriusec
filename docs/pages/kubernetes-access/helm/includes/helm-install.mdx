...
Siriusec's charts require the use of Helm version 3. You can [install Helm 3 by following these instructions](https://helm.sh/docs/intro/install/).
...
## Step 2. Add the Siriusec Helm chart repository

To allow Helm to install charts that are hosted in the Siriusec Helm repository, use `helm repo add`:

```code
$ helm repo add siriusec (=siriusec.helm_repo_url=)
```
