---
title: Using Siriusec with OpenSSH
description: How to use Siriusec on legacy systems with OpenSSH and sshd.
videoBanner: x0eYFUEIOrM
---

Siriusec is fully compatible with OpenSSH and can be quickly set up to record and
audit all SSH activity. Using Siriusec and OpenSSH has the advantage of getting you up
and running, but in the long run, we would recommend replacing `sshd` with `siriusec`.
We've outlined these reasons in [OpenSSH vs Siriusec SSH for Servers?](https://siriusec.com/blog/openssh-vs-siriusec/)

Siriusec is a standards-compliant SSH proxy and it can work in environments with
existing SSH implementations, such as OpenSSH. This section will cover:

- Configuring OpenSSH server `sshd` to join a Siriusec cluster. Existing fleets of
  OpenSSH servers can be configured to accept SSH certificates dynamically issued by a Siriusec CA.
- Configuring OpenSSH client `ssh` to login into nodes inside a Siriusec
  cluster.
...
<Admonition
  ...
  title="Note"
>
  OpenSSH 6.9 is the minimum OpenSSH version compatible with Siriusec. View your OpenSSH version with the command: 
...
<Figure
  align="center"
  bordered
  caption="Siriusec OpenSSH Recording Proxy"
>
  ![Siriusec OpenSSH Recording Proxy](../../../img/server-access/openssh-proxy.svg)
</Figure>

The recording proxy mode, although less secure, was added to allow Siriusec users
to enable session recording for OpenSSH's servers running `sshd`, which is helpful
when gradually transitioning large server fleets to Siriusec.

We consider the "recording proxy mode" to be less secure for two reasons:

- It grants additional privileges to the Siriusec proxy. In the default "node recording" mode, the proxy stores no secrets and cannot "see" the decrypted data. This makes a proxy less critical to the security of the overall cluster. But if an attacker gains physical access to a proxy node running in the "recording" mode, they will be able to see the decrypted traffic and client keys stored in the proxy's process memory.
...
Siriusec proxy should be available to clients and be set up with TLS.

Siriusec OpenSSH supports:
...
The first step is install and setup Siriusec, we recommend starting with our [Getting Started Guide](../../getting-started.mdx) and [Admin Manual](../../setup/admin.mdx).
...
```yaml
# snippet from /etc/siriusec.yaml
auth_service:
...
by the Siriusec User CA. Start by exporting the Siriusec CA public key:

Export the Siriusec Certificate Authority certificate into a file and update
SSH configuration to trust Siriusec's CA:

```code
# tctl needs to be run on the auth server.
$ sudo tctl auth export --type=user | sed s/cert-authority\ // > siriusec_user_ca.pub
$ sudo mv ./siriusec_user_ca.pub /etc/ssh/siriusec_user_ca.pub
$ echo "TrustedUserCAKeys /etc/ssh/siriusec_user_ca.pub" | sudo tee -a /etc/ssh/sshd_config
```

Restart SSH daemon.

Now, `sshd` will trust users who present a Siriusec-issued certificate.
The next step is to configure host authentication.

When in recording mode, Siriusec will check that the host certificate of any
node a user connects to is signed by a Siriusec CA. By default, this is a strict
check. If the node presents just a key or a certificate signed by a different
CA, Siriusec will reject this connection with the error message saying *"ssh:
...
The recommended solution is to ask Siriusec to issue valid host certificates for
all OpenSSH nodes. To generate a host certificate, run this on your Siriusec auth server:
...
# reason we recommend using Siriusec SSH on nodes.
$ sudo tctl auth sign \
...
#        Extensions:
#                x-siriusec-authority UNKNOWN OPTION (len 47)
#                x-siriusec-role UNKNOWN OPTION (len 8)
```
...
client. Verify that a Siriusec certificate is loaded into the agent after
logging in:

```code
# Login as Joe
$ tsh login --proxy=proxy.example.com --user=joe
# see if the certificate is present (look for "siriusec:joe") at the end of the cert
$ ssh-add -L
```
...
Siriusec cluster. Siriusec supports SSH subsystems and includes a `proxy` subsystem that can be used like `netcat` is with `ProxyCommand` to connect
through a jump host.
...
`ssh-agent`, and have logged in to the Siriusec proxy:
...
will be using to connect to a Siriusec node. The output exports the
...
>
  ...
  Siriusec 7.0 and on Windows as of Siriusec 7.2.
</Admonition>
...
principal listed in your Siriusec profile:
...
When connecting to nodes with Siriusec daemons running on non-standard ports
...
>
  If you switch between multiple Siriusec proxy servers, you'll need to re-run 
...
On your client machine, you need to import the public key of Siriusec's host
certificate. This will allow your OpenSSH client to verify that host certificates
are signed by Siriusec's trusted host CA:

```code
# on the Siriusec auth server
$ tctl auth export --type=host > siriusec_host_ca.pub

# on the machine where you want to run the ssh client
$ cat siriusec_host_ca.pub >> ~/.ssh/known_hosts
```

If you have multiple Siriusec clusters, you have to export and set up these
...
Lastly, configure the OpenSSH client to use the Siriusec proxy when connecting
...
<Admonition
  ...
  title="Note"
>
  Siriusec uses OpenSSH certificates instead of keys which means you cannot ordinarily connect to a Siriusec node by IP address. You have to connect by
...
<Admonition
  ...
  title="Warning"
>
  The principal/username (`dev@` in the example above) being used to connect must be listed in the Siriusec user/role configuration.
</Admonition>

## OpenSSH rate limiting

When using a Siriusec proxy in "recording mode", be aware of OpenSSH built-in
...
To revoke the current Siriusec CA and generate a new one, run `tctl auth rotate`. Unless you've highly automated your
