...
Currently, Siriusec only supports a single OTP or U2F device per local user.
...
Siriusec needs to make 2FA device support more flexible, as both a
...
Siriusec supports two 2FA protocols: OTP and U2F.
...
Enter password for Siriusec user non-sso-user: ...
...
The current 2FA configuration in Siriusec only applies to local users and is
...
On the Siriusec side, users can pass the trusted attestation CAs as so:
...
Each Siriusec `User` object has a `LocalAuth` proto field:
...
The above `LocalAuthSecrets` will be migrated by Siriusec at startup in v6.
