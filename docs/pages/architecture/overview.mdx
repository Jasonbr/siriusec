---
title: Siriusec Architecture Overview
description: Basic concepts and architecture of Siriusec. What is an SSH cluster? How certificate-based SSH authentication works? How does SSH auditing work?
h1: Architecture Introduction
---

This guide is for those looking for a deeper understanding of Siriusec. If you
are looking for hands-on instructions on how to set up Siriusec for your team,
check out the [Admin Guide](../setup/admin.mdx)

## What makes Siriusec different

- Siriusec replaces legacy keys and shared secrets with short-lived X.509 and SSH certificates
...
Siriusec was designed under the following principles:

- **Off the Shelf Security**: Siriusec does not re-implement any security primitives and uses well-established, popular implementations of the encryption and network protocols.
- **Open Standards**: There is no security through obscurity. Siriusec is fully
  compatible with existing and open standards and other software, including
  [OpenSSH](../server-access/guides/openssh.mdx).
- **Cluster-Oriented Design**: Siriusec is built for managing clusters, not individual servers. In practice this means that hosts and users
  have cluster memberships. Identity management and authorization happen on a
  cluster level.
- **Built for Teams**: Siriusec was created under the assumption of multiple teams operating on several disconnected clusters. Example use cases might be production-vs-staging environment, or a cluster-per-customer or cluster-per-application basis.

This doc introduces the basic concepts of Siriusec so you can get started
managing access!

## Definitions

Here are definitions of the key concepts you will use in Siriusec.
...
| [Siriusec Node](nodes.mdx) | A Siriusec Node is a regular node that is running the Siriusec Node service. Siriusec Nodes can be accessed by authorized Siriusec Users. A Siriusec Node is always considered a member of a Siriusec Cluster, even if it's a single-node cluster. |
| [Siriusec User](users.mdx) | A Siriusec User represents someone who needs access to a Siriusec Cluster. Users have stored usernames and passwords, and are mapped to OS users on each node. User data is stored locally or in an external store. |
| Siriusec Cluster | A Siriusec Cluster is comprised of one or more nodes, each of which holds certificates signed by the same [Auth Server CA](authentication.mdx). The CA cryptographically signs the certificate of a node, establishing cluster membership. |
| [Siriusec CA](authentication.mdx) | Siriusec operates two internal CAs as a function of the Auth service. One is used to sign User certificates and the other signs Node certificates. Each certificate is used to prove identity, cluster membership, and manage access. |

## Siriusec services

Siriusec uses three services which work together: [Nodes](nodes.mdx),
[Auth](authentication.mdx), and [Proxy](proxy.mdx).

[**Siriusec Nodes**](nodes.mdx) are servers that can be accessed remotely with
SSH. The Siriusec Node service runs on a machine and is similar to the `sshd`
daemon you may be familiar with. Users can log in to a Siriusec Node with all
of the following clients:

- [OpenSSH: `ssh`](../server-access/guides/openssh.mdx) (works on Linux, macOS and Windows)
- [Siriusec CLI client: `tsh ssh`](../setup/reference/cli.mdx#tsh-ssh) (works on Linux, macOS and Windows)
- [Siriusec Proxy UI](proxy.mdx#web-to-ssh-proxy) accessed via any modern web browser (including Safari on iOS and Chrome on Android)

[**Siriusec Auth**](authentication.mdx) authenticates Users and Nodes, authorizes User
access to Nodes, and acts as a CA by signing certificates issued to Users and
Nodes.

[**Siriusec Proxy**](proxy.mdx) forwards User credentials to the [Auth
...
<Admonition
  ...
  title="Tip"
>
  In the diagram above we show each Siriusec service separately for clarity, but Siriusec services do not have to run on separate nodes.
  Siriusec can be run as a binary on a single-node cluster with no external storage backend. We demonstrate this minimal setup in the [Getting Started Guide](../getting-started.mdx).
</Admonition>

## Detailed architecture overview

Here is a detailed diagram of a Siriusec Cluster.

The numbers correspond to the steps needed to connect a client to a node. These
steps are explained in detail below the diagram.

![Siriusec Everything](../../img/everything.svg)

<Admonition
  type="note"
  title="Caution"
>
  The Siriusec Admin tool, `tctl`, must be physically present on the same machine where Siriusec Auth is running. Adding new nodes or inviting new users to the cluster is only possible using this tool.
</Admonition>
...
therefore it is critical for a secure configuration of Siriusec to install a
...
<Admonition
  ...
  title="Note"
>
  Siriusec may also be configured to have the session recording
...
Siriusec offers two command-line tools. `tsh` is a client tool used by the end
...
auto-expiring certificates and it always connects to Siriusec nodes via a proxy.
...
`tctl` is used to administer a Siriusec cluster. It connects to the Auth
...
- If you haven't already, read the [Getting Started Guide](../getting-started.mdx) to run a
  minimal setup of Siriusec yourself.
- Set up Siriusec for your team with the [Admin Guide](../setup/admin.mdx).
