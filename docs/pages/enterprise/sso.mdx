---
title: SSO for SSH
description: How to set up single sign-on (SSO) for SSH using Siriusec Siriusec
h1: Single Sign-On (SSO) for SSH
---

Users of the Enterprise edition of Siriusec can login with SSH, Kubernetes, Databases and Web applications
...
Siriusec will wait for up to 3 minutes for a user to authenticate. If authentication
succeeds, Siriusec will retrieve an SSH and X.509 certificates and will store them in
`~/.tsh/keys/proxy.example.com` directory. The tool will also will add SSH cert to an
SSH agent if there's one running.

## Configuring SSO

Siriusec works with SSO providers by relying on a concept called
...
To configure SSO, a Siriusec administrator must:

- Update `/etc/siriusec.yaml` on the auth server to set the default
...
```yaml
# snippet from /etc/siriusec.yaml on the auth server:
auth_service:
...
spec:
  ...
  acs: https://siriusec-proxy.example.com:3080/v1/webapi/saml/acs
  attributes_to_roles:
    - {name: "groups", value: "okta-admin", roles: ["admin"]}
    - {name: "groups", value: "okta-dev", roles: ["dev"]}

     # note that wildcards can also be used. the next line instructs Siriusec
     # to assign "admin" role to any user who has the SAML attribute that begins with "admin":
     - { name: "group", value: "admin*", roles: ["admin"] }
     # regular expressions with capture are also supported. the next line instructs Siriusec
...
- See [examples/resources](https://github.com/siriusec/siriusec/tree/master/examples/resources)
  directory in the Siriusec Github repository for examples of possible connectors.
...
connect to Siriusec nodes. To support this:
...
- Update a Siriusec SSH role to include `{{external.unix_login}}` variable into the list of allowed logins:
...
Certain SSO providers may require or benefit from changes to Siriusec's SSO
...
- `adfs` (SAML): Required for compatibility with Active Directory (ADFS); refer
  to the full [ADFS guide](./sso/adfs.mdx#create-siriusec-roles) for details.
...
same as the first part of their email address - the 'local' part. For example, `dave.smith@acme.com` might log in with the username `dave.smith`. Siriusec provides an easy way to extract the first part of an email address so it can be used as a username - this is the `{{email.local}}` function.
...
Here's how this looks in a Siriusec role:
...
Siriusec can also support multiple connectors, i.e. a Siriusec administrator
...
| - | - | - |
| Github | `display: Github` | ![github](../../img/siriusec-sso/github@2x.png) |
| Microsoft | `display: Microsoft` | ![microsoft](../../img/siriusec-sso/microsoft@2x.png) |
| Google | `display: Google` | ![google](../../img/siriusec-sso/google@2x.png) |
| BitBucket | `display: Bitbucket` | ![bitbucket](../../img/siriusec-sso/bitbucket@2x.png) |
| OpenID | `display: Okta` | ![Okta](../../img/siriusec-sso/openId@2x.png) |

## Troubleshooting

Troubleshooting SSO configuration can be challenging. Usually a Siriusec administrator
must be able to:

- Ensure that HTTP/TLS certificates are configured properly for both Siriusec
  proxy and the SSO provider.
- Be able to see what SAML/OIDC claims and values are getting exported and passed
  by the SSO provider to Siriusec.
- Be able to see how Siriusec maps the received claims to role mappings as defined
  in the connector.

If something is not working, we recommend to:

- Double-check the host names, tokens and TCP ports in a connector definition.
- Look into Siriusec's audit log for claim mapping problems. It is usually stored on the
  auth server in the `/var/lib/siriusec/log` directory.ad
