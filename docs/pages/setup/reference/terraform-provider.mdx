...
- [siriusec_user](#siriusec_user)
- [siriusec_role](#siriusec_role)
- [siriusec_provision_token](#siriusec_provision_token)
- [siriusec_github_connector](#siriusec_github_connector)
- [siriusec_oidc_connector](#siriusec_oidc_connector)
- [siriusec_saml_connector](#siriusec_saml_connector)
- [siriusec_trusted_cluster](#siriusec_trusted_cluster)
- [siriusec_auth_preference](#siriusec_auth_preference)
...
terraform {
  required_providers {
    siriusec = {
      ...
      source  = "siriusec.com/siriusec/siriusec"
    }
  }
}
```
...
|-------------------------|------------|-----------------------------------------------------------|
|                   `addr`|     string | Siriusec auth or proxy host:port                          |
|              `cert_path`|     string | Path to Siriusec certificate file                         |
|     `identity_file_path`|     string | Path to Siriusec identity file                            |
|               `key_path`|     string | Path to Siriusec key file                                 |
|            `profile_dir`|     string | Siriusec profile path                                     |
|           `profile_name`|     string | Siriusec profile name                                     |
|           `root_ca_path`|     string | Path to Siriusec CA file                                  |
...
- `profile_name` and `profile_dir` (both can be empty) and Siriusec will try to connect using current profile from `~/.tsh`
...
The user resource allows to manage local non-interactive Siriusec users: CI/CD bots and plugins.
...
```
resource "siriusec_user" "example" {
  metadata {
    name        = "example"
    description = "Example Siriusec User"
...
|      `suggested_reviewers`| string list | List of reviewer suggestions. These can be siriusec usernames, but that is not a requirement. |
|              `annotations`|         set | Annotations                                                     |
|          `annotations.key`|      string | Annotation name                                                 |
|        `annotations.value`| string list | Annotation values                                               |
|          `claims_to_roles`|         set | Specifies a mapping from claims (traits) to siriusec roles      |
|    `claims_to_roles.claim`|      string | Claim name                                                      |
|    `claims_to_roles.roles`| string list | List of static Siriusec roles to match                          |
...
## siriusec_provision_token

Provision tokens authenticate siriusec nodes and proxies when they first join the cluster.
...
## siriusec_oidc_connector

An OIDC connector resource specifies OpenID Connect identity provider for Siriusec.
...
| `client_id`               | string      | The id of the authentication client (Siriusec Auth server)      |
...
## siriusec_saml_connector

An OIDC connector resource specifies SAML identity provider for Siriusec.
...
| Name                         | Type        | Description                                                                   |
|------------------------------|-------------|-------------------------------------------------------------------------------|
| `assertion_consumer_service` | string      | URL for assertion consumer service  on the service provider (Siriusec's side) |
...
| `service_provider_issuer`    | string      | The issuer of the service provider (Siriusec) |
...
| `attributes_to_roles.roles`  | string list | List of static Siriusec roles to map to |
...
## siriusec_auth_preference

The auth preferenfce resource defines cluster auth preference for the Siriusec cluster. This resource is a singleton. It gots reset on deletion.
