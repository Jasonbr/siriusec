...
This directory contains Docker-based flow to run Siriusec clusters locally
for testing & development purposes.

### Building 

First, you need to build `siriusec:latest` Docker image.

Run the following commands from `$GOPATH/github.com/siriusec/siriusec` (repository base dir):
...
This will start two Siriusec clusters:
...
`/var/lib/siriusec` mounted to `data/one` or `data/two` on a host. 

The configuration is passed via YAML files located in `/siriusec/docker/xxx.yaml` 
...
and then you'll find yourself inside a container where `siriusec` auth daemon
is running, try `ps -ef` for example and you'll get something like this:

```bash
container(one) /siriusec: ps -ef
UID        PID  PPID  C STIME TTY          TIME CMD
root         1     0 40 06:04 ?        00:00:06 build/siriusec start -c /siriusec/docker/one.yaml
...
1. Create a `Role` and `TrustedCluster` resource on Cluster Two.
    ...
    ```bash
    make enter-two
    tctl -c /root/go/src/github.com/siriusec/siriusec/docker/two-auth.yaml create -f docker/two-role-admin.yaml
    tctl -c /root/go/src/github.com/siriusec/siriusec/docker/two-auth.yaml create -f docker/two-tc.yaml
    ```
...
make enter-two
tctl -c /root/go/src/github.com/siriusec/siriusec/docker/two-auth.yaml auth export > docker/data/two/two.ca
exit
...
trusted_clusters:
  - key_file: /root/go/src/github.com/siriusec/siriusec/docker/data/two/two.ca
```
```bash
# update docker/two-auth.yaml with the following under "auth_service"
trusted_clusters:
  - key_file: /root/go/src/github.com/siriusec/siriusec/docker/data/one/one.ca
    allow_logins: root
    tunnel_addr: one
```
...
1. Run an simple playbook:
    ...
    cd /root/go/src/github.com/siriusec/siriusec/docker/ansible
...
Also you can start an empty container from which you can manually invoke `siriusec start`. 
This is similar to launching an empty Linux VM with a Siriusec binary.
...
Once inside, you'll get the same `/var/lib/siriusec` as "one", so you 
can start (and even build) `siriusec` daemon manually. This container also
