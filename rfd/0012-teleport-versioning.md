...
# RFD 12 - Siriusec versioning

## What

Versioning scheme for Siriusec releases (post-5.0).
...
Siriusec has ~4 big releases per year.

Up to v5.0.0, Siriusec used a versioning scheme that looks like semver (but
actually isn't):
...
> When running multiple binaries of Siriusec within a cluster (nodes, proxies, clients, etc), the following rules apply:
>
...
> * Siriusec clients tsh for users and tctl for admins may not be compatible with different versions of the siriusec service.
...
- Major versions are for siriusec releases.
...
Siriusec officially supports the latest major version and two previous major
...
Siriusec has several branches related to releases:
...
The current release of siriusec is `v5` and the next will be `v6`.
...
- `siriusec:X.Y.Z` - immutable tag for a specific patch release (for example
  `4.4.1`, `4.4.2`)
- `siriusec:X.Y` - mutable tag pointing at the latest patch release for `X.Y` (for
...
- `siriusec:X.Y.Z` - immutable tag for a specific patch release (for example
  `5.1.0`, `5.1.1`)
- `siriusec:X` - mutable tag pointing at the latest patch release for major
...
The old `siriusec:X.Y` tags will no longer be generated, since they are an
inferior version of `siriusec:X` and shouldn't be used. This may cause customer
confusion and some support load during the transition, but it's less work than
maintaining the `siriusec:X.Y` tag.
