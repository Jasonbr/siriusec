---
authors: Andrej Tokarčík (andrej@gosiriusec.com)
state: draft
---
...
as to which branches of these scenarios are to be supported by Siriusec.
...
The current behaviour is that Siriusec creates dynamic configuration implicitly
...
1. To faciliate automated/programmatic management of Siriusec clusters.

2. To add configuration capability to Siriusec-as-a-service offerings that do
   not allow direct access to `siriusec.yaml` files.

## Scenarios

We use the example of the `authentication` subsection of `auth_service`
in `siriusec.yaml` (as "the" static configuration) and the corresponding
...
1. The `authentication` section is specified in `siriusec.yaml`.
...
3. `tctl get cap` returns the resource data in line with `siriusec.yaml`.
...
1. The `authentication` section has never been specified in `siriusec.yaml`.
...
1. The `authentication` section used to be specified in `siriusec.yaml` but
...
5. Upon the next auth server restart with `authentication` still unspecified
   in `siriusec.yaml`, the auth server initialization procedure retains
...
The resource label `siriusec.dev/origin` (below shortened to `origin` for
...
|       :---:       |            :---:            |                 :---:                |              :---:              |
|   **`defaults`**  |             n/a             | specify in `siriusec.yaml` & restart |          `tctl create`          |
| **`config-file`** | remove from `siriusec.yaml` |  change in `siriusec.yaml` & restart | `tctl create --force --confirm` |
|   **`dynamic`**   |          `tctl rm`          | specify in `siriusec.yaml` & restart |      `tctl create --force`      |

The `siriusec.dev/origin` label is reserved for system use.  Resources derived
from `siriusec.yaml` are necessarily labelled either as `defaults` (when the
...
1. If `siriusec.yaml` section is specified, store it as a resource in the
   backend with `origin: config-file`.

2. If `siriusec.yaml` section is not specified, attempt to fetch the resource
...
4. If called with `--force` and the fetched resource has `origin: config-file`
   ...
   configuration from siriusec.yaml, restarting the servers and trying this
   command again.
...
3. If the fetched resource has `origin: config-file` then print an error:
   ```
   This resource is managed by static configuration. We recommend removing
   configuration from siriusec.yaml and restarting the servers in order to
