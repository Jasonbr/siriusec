...
description: How to configure SSH access with Google Workspace (also known as G Suite) using Siriusec
...
- An Enterprise version of Siriusec downloaded from [https://dashboard.siriusec.com/](https://dashboard.siriusec.com/web/login).
...
5. Copy OAuth Client ID and Client Secret for YAML Below.
   Note: The redirect_url: `https://siriusec.example.com:3080/v1/webapi/oidc/callback`
...
<Admonition type="note">
  Siriusec requires the service account JSON to be uploaded to all Siriusec authentication servers when setting
...
The second is populating the contents via inline.  Inline is required for Siriusec Cloud.
...
<Tabs>
  ...
  <TabItem label="Google Service Account via inline (required for Siriusec Cloud)">
    ```yaml
...
<Admonition
  ...
  title="Tip"
>
  Siriusec can use multiple OIDC connectors. In this case a connector name
