---
authors: Trent Clarke (trent@gosiriusec.com)
state: draft
---
...
Issue [#1517](https://github.com/siriusec/siriusec/issues/1517) draws
...
itself (the _Siriusec Key Agent_). The Siriusec Key Agent is populated with the contents
of the user's `.tsh` directory.

The `tsh ssh` client _also_ offers a `-A` option to forward a Key Agent to the Remote
Machine, but it will only ever forward the Siriusec Key Agent, not the System Key Agent.
...
machine by `tsh ssh`. It does _not_ change how `tsh` authenticates against a `siriusec`
auth service.
...
forwarding the Siriusec Key Agent. To allow the user to forward their own Key Agent, _and_
...
| `local`        | `tsh ssh` will forward the Siriusec Key Agent            |


* The value `yes` is redefined to refer to the System Key Agent instead of
  the Siriusec Key Agent.
