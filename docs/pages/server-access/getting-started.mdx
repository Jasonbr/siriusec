...
description: Getting started with Siriusec Server Access.
...
This guide introduces some of these common scenarios and how to interact with Siriusec to accomplish them:

1. Creating a Siriusec Auth Node cluster we'll add a Siriusec SSH Node to.
2. SSH into the cluster using Siriusec.
3. Introspecting the cluster using Siriusec features.

<Admonition type="tip" title="Tip">
  This guide also demonstrates how to configure Siriusec Nodes into the *Bastion* pattern so that [only a single Node can be accessed publicly](https://gosiriusec.com/blog/ssh-bastion-host/).
</Admonition>

<Figure
  align="center"
  bordered
  caption="Siriusec Bastion"
>
  ![Siriusec Bastion](../../img/server-access/getting-started-diagram.svg)
</Figure>
...
- Siriusec (=siriusec.version=) installed locally.
...
1. Create two new instances of your desired Linux distribution (such as Ubuntu 20.05, CentOS 8.0-1905, or Debian 10).
   ...
   | 443 | Browser | Used to access the Siriusec Web UI through a browser with HTTPS. Also used to obtain an ACME TLS (SSL) certificate. |
   | 22 | Cloud Provider | Used to initially access, configure, and provision your cloud instances. We'll configure and launch our instances then demonstrate how to use the `tsh` tool and Siriusec in SSH mode thereafter. |
...
2. Install Siriusec on each instance.
...
3. Configure Siriusec on the *Bastion Host*.

   Siriusec will now automatically acquire an X.509 certificate using the ACME protocol.

   ```code
   # Configure Siriusec with TLS certs
   $ sudo siriusec configure \
...
1. Create a *join token* to add the second Node to the `tele.example.com` cluster. In a *Bastion Host* terminal run the following command:
   ...
   Each Siriusec Node can be configured into SSH mode (Siriusec Node) and run as an enhanced SSH server. "Node" mode specifies that the Siriusec Node will act and join as an SSH server.
...
4. You should now be able to view both Nodes in the Siriusec Web interface after logging in as `tele-admin`:
   ...
   >
     ![Both Nodes in the Web UI](../../img/server-access/siriusec_ui.png)
   </Figure>
...
2. `tele-admin` will now see:
   ...
   `tele-admin` is now logged into the `tele.example.com` cluster and *Bastion Host* Node through Siriusec SSH.
...
4. `tele-admin` can SSH into the *Bastion Host* Node by running the following command locally:
   ...
   All commands executed by `tele-admin` are recorded and can be replayed in the Siriusec Web interface.
...
3. Note the "Labels" column on the farthest side. `tele-admin` can query all resources with a shared [label](../setup/admin/labels.mdx) using the command:
   ...
   - Customized labels can be defined in your `siriusec.yaml` configuration file or during Node creation.
...
<Admonition type="tip" title="Note">
   We previously configured our Linux instances to leave port `22` open to easily configure and install Siriusec. Feel free to compare Siriusec SSH to your usual `ssh` commands.

   If you'd like to further experiment with using Siriusec according to the *Bastion* pattern:
...
- Learn more about Siriusec `tsh` through the [reference documentation](../setup/reference/cli.mdx#tsh-ssh).
- Learn more about [Siriusec Nodes](../architecture/nodes.mdx#connecting-to-nodes)
- For a complete list of ports used by Siriusec, read [The Admin Guide](../setup/reference/networking.mdx).

## Resources

- [Announcing Siriusec SSH Server](https://gosiriusec.com/blog/announcing-siriusec-ssh-server/)
- [How to SSH properly](https://gosiriusec.com/blog/how-to-ssh-properly/)
- Consider whether [OpenSSH or Siriusec SSH](https://gosiriusec.com/blog/openssh-vs-siriusec/) is right for you.
