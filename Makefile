...
DOCKER_IMAGE ?= registry.siriusec.com/siriusec
DOCKER_IMAGE_CI ?= registry.siriusec.com/siriusec-ci
...
FIPS ?=
RELEASE = siriusec-$(GITTAG)-$(OS)-$(ARCH)-bin
...
.PHONY: $(BUILDDIR)/sctl
$(BUILDDIR)/sctl:
	GOOS=$(OS) GOARCH=$(ARCH) $(CGOFLAG) go build -tags "$(PAM_TAG) $(FIPS_TAG) $(BPF_TAG)" -o $(BUILDDIR)/sctl $(BUILDFLAGS) ./tool/sctl

.PHONY: $(BUILDDIR)/siriusec
$(BUILDDIR)/siriusec: ensure-webassets bpf-bytecode
	GOOS=$(OS) GOARCH=$(ARCH) $(CGOFLAG) go build -tags "$(PAM_TAG) $(FIPS_TAG) $(BPF_TAG) $(WEBASSETS_TAG)" -o $(BUILDDIR)/siriusec $(BUILDFLAGS) ./tool/siriusec
...
goinstall:
	go install $(BUILDFLAGS) \
		github.com/siriusec/siriusec/tool/tsh \
		github.com/siriusec/siriusec/tool/siriusec \
		github.com/siriusec/siriusec/tool/sctl

# make install will installs system-wide siriusec
.PHONY: install
install: build
	@echo "\n** Make sure to run 'make install' as root! **\n"
	cp -f $(BUILDDIR)/sctl      $(BINDIR)/
	cp -f $(BUILDDIR)/tsh       $(BINDIR)/
	cp -f $(BUILDDIR)/siriusec  $(BINDIR)/
	mkdir -p $(DATADIR)
