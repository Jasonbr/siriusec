---
title: Ansible
description: Using Siriusec with Ansible
---

Ansible uses the OpenSSH client by default. Siriusec supports SSH protocol and
works as SSH jumphost.

In this guide we will configure OpenSSH client to work with Siriusec Proxy
...
- `tsh` client tool  >= (=siriusec.version=)
- `ssh` openssh tool
...
to a valid SSH username that works with the target host and is allowed by Siriusec:
...
You are all set. You are now using short-lived SSH certificates and Siriusec can now record
