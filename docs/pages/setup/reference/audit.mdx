...
description: Reference of Siriusec Audit Events and Session Records
---

Siriusec logs every SSH event into its audit log. There are two components of
the audit log:

1. **Cluster Events:** Siriusec logs events like successful user logins along with the metadata like remote IP address, time, and the session ID.
...
Refer to the ["Audit Log" chapter in the Siriusec
...
Siriusec supports multiple storage back-ends for storing the SSH, Application, and Kubernetes events.
...
Let's examine the Siriusec audit log using the `dir` backend. The event log is
stored in `data_dir` under `log` directory, usually `/var/lib/siriusec/log` .
Each day is represented as a file:

```code
$ ls -l /var/lib/siriusec/log/
...
In addition to logging `session.start` and `session.end` events, Siriusec also records the entire stream of bytes going to/from standard input and standard the output of an SSH session.

Siriusec can store the recorded sessions in an [AWS S3 bucket](./backends.mdx#s3)
