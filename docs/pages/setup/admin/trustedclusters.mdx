---
title: Siriusec Trusted Clusters
description: How to configure access and trust between two SSH and Kubernetes environments.
h1: Trusted Clusters
---

The design of trusted clusters allows Siriusec users to connect to compute infrastructure
...
- Establish permissions mapping between clusters using Siriusec roles.

<Admonition
  type="tip"
  title="Siriusec Node Tunneling"
>
  If you have a large number of devices on different networks, such as managed IoT devices or a couple of nodes on a different network you can utilize the [Siriusec Node Tunneling](./adding-nodes.mdx#adding-a-node-located-behind-nat).
</Admonition>

## Introduction

As explained in the [architecture document](../../architecture/overview.mdx#design-principles),
Siriusec can partition compute infrastructure into multiple clusters.
...
The concept of *leaf clusters* allows Siriusec administrators to connect
...
![Siriusec Cluster Page](../../../img/trusted-clusters/siriusec-trusted-cluster.png)
...
Users of Siriusec will recognize that this is the same way you would add any
...
spec:
  ...
  # (enterprise editions of Siriusec only)
  role_map:
...
>
  ...
  ease of testing, the Siriusec daemon on "leaf" can be started with the `--insecure` CLI flag to accept self-signed certificates. Make sure to configure
...
"root" should be allowed in and what permissions should they have. Siriusec offers
...
- Every user in Siriusec Enterprise is assigned a role.
...
For customers using Siriusec Enterprise, they can easily configure *leaf* nodes using the
Siriusec Proxy UI.
...
Siriusec gives administrators of root clusters the ability to control cluster labels.
...
- Metadata (certificate extensions): additional data protected by the signature above. Siriusec uses the metadata to store the list of user roles and SSH
...
There are three common types of problems Siriusec administrators can run into when configuring
...
testing, the siriusec daemon on "leaf" can be started with the `--insecure` CLI flag to accept
...
`siriusec start --debug`. You can also do this by updating the configuration
...
Troubleshooting access denied messages can be challenging. A Siriusec administrator
...
- Which roles a user is assigned on "leaf" when the role mapping takes place.
  The role mapping result is reflected in the Siriusec audit log. By default,
  it is stored in `/var/lib/siriusec/log` on a *auth* server of a cluster.
