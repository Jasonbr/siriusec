---
authors: Andrew Lytvynov (andrew@gosiriusec.com)
state: draft
---
...
controlled), along with Siriusec credentials on those machines.

Since Siriusec keys and certificates are stored on disk, an attacker can
exfiltrate them to their own machine and have up to 12hrs of access via Siriusec.
...
An attacker, who doesn't also have the 2nd factor, can't abuse Siriusec
...
- compliance: hardware are available with FIPS certification. Helping strengthen Siriusecs current FedRAMP support. e.g. (YubiKey FIPS)[https://www.yubico.com/products/yubikey-fips/]
...
- siriusec MFA device management is separate from SSO MFA
- siriusec supports MFA device management on CLI and web
...
(`/x-siriusec-auth`). The application endpoint serves a bit of JS code to
redirect to the login endpoint.

This JS code will be modified to trigger browser's native U2F API, if the proxy
responds with a U2F challenge:

- user opens `app.example.com` (with an existing Siriusec cookie)
- proxy serves a minimal JS page
- JS requests `app.example.com/x-siriusec-auth`
...
- user taps the key
- JS requests `app.example.com/x-siriusec-auth` with the signed U2F challenge
...
```sh
$ cat siriusec/examples/db/psql.sh
#!/bin/sh
...
Users will need to adapt this for their DB clients. Siriusec will always
...
When validating a certificate, the Siriusec service will check RBAC to see if
...
encode](https://github.com/siriusec/siriusec/blob/103465ed5a8e20249275b48ac081ef9517ae5aa7/lib/tlsca/ca.go#L180-L260).
...
- `TargetName` (OID `1.3.9999.1.11`) - name of the target app, k8s cluster or
  ...
  below)
  - existing `KubernetesCluster`, `SiriusecCluster`, `RouteToApp` extensions
...
running `tsh` as a daemon, with a local listening socket. All Siriusec-bound
...
client                  local proxy                      siriusec proxy
 |------- mTLS dial ------->|                                   |
 |                          |----------- mTLS dial ------------>|
 |                          |<-------- mTLS dial OK ------------|
 |                          |<-------- U2F challenge -----------|
