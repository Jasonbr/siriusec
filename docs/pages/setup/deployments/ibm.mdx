---
title: Running Siriusec on IBM Cloud
description: How to install and configure Siriusec Siriusec on IBM cloud for SSH and Kubernetes access.
---
...
- [Siriusec on IBM FAQ](#siriusec-on-ibm-cloud-faq)
- [IBM Siriusec Introduction](#ibm-siriusec-introduction)

## Siriusec on IBM Cloud FAQ

### Why would you want to use Siriusec with IBM Cloud?

Siriusec provides privileged access management for cloud-native infrastructure that
...
By using Siriusec with IBM you can easily unify all access for both IBM Cloud and
...
We plan on expanding our guide to eventually include using Siriusec with IBM Cloud Kubernetes Service.

## IBM Siriusec Introduction

This guide will cover how to setup, configure and run Siriusec on IBM Cloud.

IBM Services required to run Siriusec in High Availability:
...
We recommend setting up Siriusec in High Availability mode (HA). In High Availability mode [etcd](https://etcd.io/)
...
Siriusec uses etcd as a scalable database to maintain High Availability and provide 
...
We recommend picking an etcd instance in the same region as your planned Siriusec
cluster.

- Deployment region: Same as rest of Siriusec Cluster
...
siriusec:
  storage:
    ...
    tls_ca_file: '/var/lib/siriusec/797cfsdf23e-4027-11e9-a020-42025ffb08c8.pem'
...
We recommend using [IBM Cloud File Storage](https://www.ibm.com/cloud/file-storage) to store Siriusec recorded sessions.
...
<Admonition type="tip">
  When starting with `siriusec start --config=/etc/siriusec.yaml -d` you can confirm that the
  bucket has been created.
</Admonition>

```code
$ siriusec start --config=/etc/siriusec.yaml -d
# DEBU [SQLITE]    Connected to: file:/var/lib/siriusec/proc/sqlite.db?_busy_timeout=10000&_sync=OFF, poll stream period: 1s lite/lite.go:173
...
# INFO [S3]        Setting up bucket "ben-siriusec-test-cos-standard-b98", sessions path "/readonly/records" in region "ibm". s3sessions/s3handler.go:143
# INFO [S3]        Setup bucket "ben-siriusec-test-cos-standard-b98" completed. duration:356.958618ms s3sessions/s3handler.go:147
```

### Network: IBM Cloud DNS Services

We recommend using [IBM Cloud DNS](https://cloud.ibm.com/catalog/services/dns-services) for
the Siriusec Proxy public address.
