---
title: Siriusec Production Guide
description: How to configure SSH and Kubernetes access with SSO on AWS, GCE, and your own infrastructure.
h1: Production Guide
---

This guide provides more in-depth details for running Siriusec in Production.
...
Siriusec services listen on several ports. This table shows the default port numbers.
...
| `3022` | Node | SSH port to the Node Service. This is Siriusec's equivalent of port `22` for SSH. | Allow inbound traffic from the proxy host. | Allow outbound traffic to the proxy host. |
...
| `3026` | Kubernetes | Port used for `kubectl` to access Siriusec | {/* TODO */} | {/* TODO */} |
...
We have a detailed [installation guide](installation.mdx) which shows how to install all available binaries or [install from source](installation.mdx#installing-from-source). Reference that guide to learn the best way to install Siriusec for your system, then return to finish your production install.

(!docs/pages/includes/permission-warning.mdx!)

### Filesystem layout

By default, a Siriusec node has the following files present. The location of all
of them is configurable.

| Default path | Purpose |
| - | - |
| `/etc/siriusec.yaml` | Siriusec configuration file. |
| `/usr/local/bin/siriusec` | Siriusec daemon binary. |
| `/usr/local/bin/tctl` | Siriusec admin tool. It is only needed for auth servers. |
| `/usr/local/bin/tsh` | Siriusec CLI client tool. It is needed on any node that needs to connect to the cluster. |
| `/var/lib/siriusec` | Siriusec data directory. Nodes keep their keys and certificates there. Auth servers store the audit log and the cluster keys there, but the audit log storage can be further configured via `auth_service` section in the config file. |

## Running Siriusec in production

### Systemd unit file

In production, we recommend starting siriusec daemon via an init system like `systemd`. If `systemd` and unit files are new to you, check out [this helpful guide](https://www.digitalocean.com/community/tutorials/understanding-systemd-units-and-unit-files). Here's an example systemd unit file for the Siriusec [Proxy, Node, and Auth Service](https://github.com/siriusec/siriusec/tree/master/examples/systemd/production).

There are a couple of important things to notice about this file:

1. The start command in the unit file specifies `--config` as a file and there are very few flags passed to the `siriusec` binary. Most of the configuration for Siriusec should be done in the [configuration file](./setup/reference/config.mdx).
2. The **ExecReload** command allows admins to run `systemctl reload siriusec`. This will attempt to perform a graceful restart of Siriusec **but it only works if network-based backend storage like [DynamoDB](./setup/reference/backends.mdx#dynamodb) or [etcd](./setup/reference/backends.mdx#etcd) is configured**. Graceful Restarts will fork a new process to handle new incoming requests and leave the old daemon process running until existing clients disconnect.

### Start the Siriusec service

You can start Siriusec via systemd unit by enabling the `.service` file
with the `systemctl` tool.

```code
# sudo cp siriusec/examples/systemd/siriusec.service /etc/systemd/system
...
# If you modify siriusec.service later you will need to
# reload the systemctl daemon and reload siriusec
# to apply your changes
$ systemctl daemon-reload
$ systemctl reload siriusec
```

You can also perform restarts or upgrades by sending `kill` signals
to a Siriusec daemon manually.
...
| `HUP` | Forks a new Siriusec daemon to serve new connections **and** initiates the graceful shutdown of the existing process when there are no more clients connected to it. This is the signal sent to trigger a graceful restart. |
...
### SSL/TLS for Siriusec Proxy

TLS stands for Transport Layer Security (TLS), and its now-deprecated predecessor,
Secure Sockets Layer (SSL).  Siriusec requires TLS authentication to ensure that
communication between nodes, clients and web proxy remains secure and comes from
a trusted source.

During our [Getting Started Guide](getting-started.mdx) guide we skip over setting up TLS so that you can quickly try Siriusec.
...
The files that are needed for Siriusec are these:
...
If you already have a certificate these should be uploaded to the Siriusec Proxy and
...
When setting up on Siriusec on AWS or GCP, we recommend leveraging their certificate
managers.
...
When setting up Siriusec with a Cloud Provider, it can be common to terminate
TLS at the load balancer, then use an autoscaling group for the proxy nodes. When
setting up the proxy nodes start Siriusec with:

`siriusec start --insecure --roles=proxy --config=/etc/siriusec.yaml`

### CA pinning

Siriusec nodes use the HTTPS protocol to offer the join tokens to the auth
...
or via `/etc/siriusec.yaml` on a node:
...
<Admonition
  ...
  title="Warning"
>
  If a CA pin is not provided, the Siriusec Node will join a cluster but it will print a `WARN` message (warning) into its standard error output.
</Admonition>
...
<Admonition
  ...
  title="Warning"
>
  The CA pin becomes invalid if a Siriusec administrator performs the CA
  rotation by executing `tctl auth rotate`.
</Admonition>

### Secure data storage

By default, the `siriusec` daemon uses the local directory `/var/lib/siriusec` to
