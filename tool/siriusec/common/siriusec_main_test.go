...
// bootstrap check
func TestSiriusecMain(t *testing.T) {
	// get the hostname
	hostname, err := os.Hostname()
	require.NoError(t, err)

	// generate the fixture config file
	configFile := filepath.Join(t.TempDir(), "siriusec.yaml")
	err = ioutil.WriteFile(configFile, []byte(YAMLConfig), 0660)
	require.NoError(t, err)

	// set defaults to test-mode (non-existing files&locations)
	defaults.ConfigFilePath = "/tmp/siriusec/etc/siriusec.yaml"
	defaults.DataDir = "/tmp/siriusec/var/lib/siriusec"
	...
	t.Run("Default", func(t *testing.T) {
		...
		require.Equal(t, "/tmp/siriusec/var/lib/siriusec", conf.DataDir)
