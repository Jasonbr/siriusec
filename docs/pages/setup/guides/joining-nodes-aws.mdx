...
This section will explain how to set up Siriusec Nodes and Proxies to join your
Siriusec cluster with the EC2 join method. The EC2 join method enables you to
allow Siriusec nodes to join your Siriusec cluster from EC2 instances in your
AWS account without sharing any secrets.

## Prerequisites

- Siriusec v(=siriusec.version=) Open Source or Enterprise.
- A running self-hosted Siriusec cluster.
- AWS EC2 instance with Siriusec installed.

## Step 1/4. Setup AWS permissions for your Auth server

Siriusec Auth needs permission to call `ec2:DescribeInstances` in order to check
that the EC2 instances attempting to join your cluster are legitimate and
currently running.

### Step 1.1. Create the IAM policy

Create the following AWS IAM policy named `siriusec-DescribeInstances-policy` in
your account:
...
If your Siriusec auth server is running on an EC2 instance and already has an
attached "IAM role for Amazon EC2", add the above
`siriusec-DescribeInstances-policy` to the existing role. If the instance
does not already have an attached role, create an IAM role with the above
policy and attach it to your EC2 instance running the Siriusec auth server.

If you are running your Siriusec auth server outside of AWS you can attach
the `siriusec-DescribeInstances-policy` directly to an IAM user which
Siriusec will use to authenticate. You can provide the IAM credentials to
siriusec through a shared configuration file or environment variables, see
https://aws.github.io/aws-sdk-go-v2/docs/configuring-sdk/#specifying-credentials
for details.

## Step 2/4. Create the AWS Node Joining Token

Configure your Siriusec auth server with a special dynamic token which will
allow nodes from your AWS account to join your Siriusec cluster. Nodes will
...
The EC2 join method can be used for Siriusec services running SSH, Proxy,
...
Configure your Siriusec node with a custom `siriusec.yaml` file. Use the
...
## Step 4/4. Launch your Siriusec Node
...
to get Siriusec set up and configured. This feature works best once Siriusec is
...
Start Siriusec on the node and confirm that it is able to connect to and join
...
In order for Siriusec nodes to join from EC2 instances in AWS accounts other
than the account in which your Siriusec auth server is running, Siriusec must
...
1. Create the `siriusec-DescribeInstances-policy` from [Step 1.1](#step-11-create-the-iam-policy).

2. Create an IAM role `siriusec-DescribeInstances-role` that can be assumed from
   the account where your Siriusec auth server is running.
   ...
   Attach the `siriusec-DescribeInstances-policy` to the role.

In the AWS account where your Siriusec auth server is running:

1. Create an IAM policy named `siriusec-AssumeRole-policy` with an `AssumeRole`
   ...
   "Statement": [
        {
            ...
            "Resource": "arn:aws:iam::222222222222:role/siriusec-DescribeInstances-role"
        },
        {
            "Effect": "Allow",
            "Action": "sts:AssumeRole",
            "Resource": "arn:aws:iam::333333333333:role/siriusec-DescribeInstances-role"
        }
   ]
}
```

2. Attach this `siriusec-AssumeRole-policy` to the IAM role your Siriusec auth
...
`siriusec-DescribeInstances-role`.
...
spec:
  ...
  - aws_account: "222222222222"
    ...
    - us-west-1
    - us-west-2
    aws_role: "arn:aws:iam::222222222222:role/siriusec-DescribeInstances-role"
  - aws_account: "333333333333"
    aws_regions:
    - us-west-1
    - us-west-2
    aws_role: "arn:aws:iam::333333333333:role/siriusec-DescribeInstances-role"
```
