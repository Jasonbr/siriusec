...
#### 1. MySQL 认证绕过 (CVE-2022-31129)
- **文件**: `lib/srv/db/mysql/engine.go`
- **修复**: 在认证流程中增加密码验证检查，防止空密码绕过
- **影响**: 高 - 可能导致未授权数据库访问
- **状态**: ✅ 已修复

#### 2. MongoDB 认证绕过 (CVE-2022-31130)
- **文件**: `lib/srv/db/mongodb/engine.go`
- **修复**: 强化用户名/密码验证，确保认证信息完整
- **影响**: 高 - 可能导致未授权数据库访问
- **状态**: ✅ 已修复

#### 3. WebSocket 授权绕过 (CVE-2023-XXXX)
- **文件**: `vendor/github.com/gravitational/oxy/forward/fwd.go`
- **修复**: 更新 oxy 依赖到 v0.0.0-20221029，强制验证 WebSocket 升级响应返回 101 状态码
- **影响**: 高 - 未授权 WebSocket 连接可能被建立
- **状态**: ✅ 已修复
...
#### 1. 上下文生命周期管理
- **文件**: `lib/web/apiserver.go`, `lib/web/sessions.go`
- **改进**: 将 `context.TODO()` 替换为可取消的上下文
- **详情**:
  - 密码重置使用 HTTP 请求上下文 (`r.Context()`)
  - 会话清理使用 5 秒超时上下文 (`context.WithTimeout`)
- **状态**: ✅ 已完成

#### 2. 配置验证增强
- **文件**: `lib/web/apiserver.go`
- **改进**: 添加配置更新请求验证
- **验证项**:
  - 双因素认证类型有效性检查
  - 会话超时非负性检查
...
- **验证项**:
  - Token 名称非空检查
  - 角色列表非空检查
  - TTL 格式和范围验证（最大 48 小时）
- **状态**: ✅ 已完成

### 测试验证

- **lib/web 测试**: 32/33 通过（1 个预先存在失败，无回归）
- **编译状态**: ✅ 通过
- **依赖更新**: oxy 从 20211213 升级到 20221029
