---
authors: Sasha Klizhentas (sasha@gosiriusec.com)
state: draft
---
...
Please save these account recovery tokens for example.siriusec.sh in a safe offline place.
...
Siriusec cloud scanners can look for `tele-` prefix and alert customers with
...
* [ ] Siriusec should store tokens hashed with `crypto/bcrypt` and compare using constant-time compare.
Siriusec should store the time when the tokens were generated alongside the hashed tokens.
...
on Siriusec system administrators as well.

**Emails and usernames**

Siriusec Cloud assumes that username is a user email.
When sending emails, Siriusec cloud will send an email to the username if it contains @ sign.
Self hosted Siriusec will not send any emails.
...
Review [Access requests with dual authorization](https://gosiriusec.com/docs/access-controls/guides/dual-authz/)
for more details on access requests to understand this RFD better.

Siriusec will create `account-owner` and `account-editor` role presets.
...
Siriusec's access requests by do not allow any user to review their own requests, so the
...
This feature relies on not-implemented yet [session and user locks](https://github.com/siriusec/siriusec/issues/3360).
We will assume that `lock` is a new resource that can place a lock on account or a set of users.

Siriusec cloud will create a `locksmith` and and `lock-editor` role presets.
...
* Siriusec Cloud generates a recovery link and emails this link to the user's email.

```
This is Siriusec cloud account recovery link initiated <date> from <device> and <location>.
...
Siriusec cloud sends a recovery link to an email address to protect from a physical compromise -
someone stealing the laptop and the printed tokens.

Recovery triggers an email notification to a user:

```
Your Siriusec Cloud account has been successfully recovered on <date> from <device> and <location>.
...
```
Siriusec Cloud account for user@example has been successfully recovered on <date> from <device> and <location>.
No further action is necessary.
```
...
regeneration Siriusec has to confirm that user is near their computer and actively authorizes the action.

If a user has logged in the last 5 minutes, Siriusec asks for any valid second factor
token authorization.

If the login hasn't been done in the last 5 mins, Siriusec asks Alice to re-login completely -
...
If any user is logged into Siriusec UI, they can regenerate new password recovery tokens:

* Alice goes to her account settings and chooses to generate a new set of recovery tokens.
* Siriusec asks for a second factor using *Authorizing privileged operations flow* before generating tokens and makes it clear that once
...
* Log into Siriusec UI.
* Navigate to account details page and request account cancellation.
* Siriusec asks Bob prove the ownership of a valid second factor using *Authorizing privileged operations* flow.
...
To proceed with initiation, log-in to Siriusec UI and approve the operation.
Otherwise, do nothing and the request will expire in 1 hour.
```

Behind the scenes, Siriusec creates access request to get the role `account-editor` with TTL of 1 hour.

A second account owner, Alice, has to log into Siriusec to approve/deny the operation.

* Alice logs into Siriusec and approves the operation using existing access request UI or CLI.
  + If Alice denies the operation Siriusec uses existing `DENY` operation on access requests.
* Siriusec asks to prove Alice's ownership of a valid second factor using *Authorizing privileged operations* flow.
* Bob receives the notification that request was approved or denied and has to re-login in to Siriusec UI to assume
  role or use the "Assume role" existing user interface.
* Bob can press "cancel" using *Authorizing privileged operations* flow.

Siriusec sends an email:
...
* Siriusec should not disconnect any resources connected to the cloud account during the grace period.
...
```
Your account was marked as inactive at the request of `email`.
To prevent account from being deleted, login into siriusec, go to `My Account` and activate `Revert account cancellation`.
```
...
Siriusec will ask a logged in account owner for a second factor proof to revert account cancellation to prevent
...
In this case Siriusec cloud will approve the access request. This creates a delete user attack vector -
...
Siriusec will encourage to set up two account owners using notifications and separate privileges.
...
Siriusec cloud support should be able to delay account cancellation by 10 days if they see a zendesk
...
* In case if there is only one account owner, Siriusec approves access request
...
By session lock refer to a [session lock](https://github.com/siriusec/siriusec/issues/3360) feature
that is not implemented yet.

To implement dual authorization this feature will rely on existing [Access requests with dual authorization](https://gosiriusec.com/docs/access-controls/guides/dual-authz/).
...
* Siriusec verifies second factor using Siriusec asks for a second factor using *Authorizing privileged operations flow*.
* Siriusec creates an access request for a user requesting a locksmith role.
...
To proceed with initiation, log-in to Siriusec UI and confirm the lock.
Otherwise, do nothing and the request will expire in 1 hour.
```

Bob, a second locksmith logs into Siriusec and reviews the request using existing access requests flow.
...
```
Siriusec cloud account <name> has only one owner.

To improve security and prevent account loss please add a second account owner.

Please check documentation: https://gosiriusec.com/docs/cloud/owners for more details.
```
...
Please check documentation: https://gosiriusec.com/docs/cloud/locksmiths for more details.
```
...
Please check documentation: https://gosiriusec.com/docs/cloud/least-privilege for more details.
```
