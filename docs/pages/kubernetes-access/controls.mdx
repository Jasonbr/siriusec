---
title: Siriusec Kubernetes Access Controls
description: Connect SAML and OIDC users to Kubernetes RBAC with Siriusec
---

# Single Sign-On and Kubernetes RBAC

Siriusec issues short-lived X.509 certs and updates Kubernetes clients to talk to Siriusec proxy using mutual TLS.
...
Siriusec's connectors configure single sign-on for OIDC, OAuth, or SAML providers.
Here are examples of connectors that map SSO users to Kubernetes groups in the Siriusec Open Source Edition and Siriusec's roles in the Enterprise Edition:
...
<Tabs>
  ...
  <TabItem label="Enterprise">
    ...
    attribute statements and claims received from identity providers to Siriusec's roles.
    ...
    spec:
      ...
      # will be given the Siriusec `admin` role.
...
is shared with Siriusec. Siriusec receives user metadata keys and values as OIDC claims or SAML
attributes during [signle sign-on redirect flow](https://gosiriusec.com/blog/how-oidc-authentication-works/):
...
Siriusec's roles map OIDC claims or SAML attributes using template variables.
...
<Tabs>
  <TabItem label="Helm">
    ...
    $ helm upgrade siriusec-agent siriusec-kube-agent --set kubeClusterName={CLUSTER?}\
...
Siriusec uses [Kubernetes impersonation](https://kubernetes.io/docs/reference/access-authn-authz/authentication/#user-impersonation)
...
allowing Siriusec's `ServiceAccount` to impersonate users and groups:

```yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata:
  name: siriusec-impersonation
rules:
- apiGroups:
  - ""
  resources:
...
- kind: ServiceAccount
  ...
  name: siriusec-serviceaccount
  namespace: default
```

Take a look at the example usage in a [Siriusec Helm chart](https://github.com/siriusec/siriusec/blob/v\(=siriusec.version=\)/examples/chart/siriusec-cluster/templates/clusterrole.yaml)
