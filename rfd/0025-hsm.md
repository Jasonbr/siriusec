---
authors: Andrew Lytvynov (andrew@gosiriusec.com), Nic Klaassen
(nic@gosiriusec.com)
state: draft
---
...
First, some background on HSMs and Siriusec CAs.
...
Each Siriusec cluster has multiple CAs:
- Host CA (SSH + TLS)
- User CA (SSH + TLS)
- JWT CA (TLS only)
...
1 CA of each type is deeply integrated in Siriusec.

### Architecture

Each Auth server may have an HSM device available, configured in
`siriusec.yaml`. Auth servers create and manage unique CA keys in their local
...
Current [CA rotation
logic](https://github.com/siriusec/siriusec/blob/8cb3ba36b5734b9212d7fe9bdc255794b31d7a1d/lib/auth/rotate.go#L133-L205)
...
message CertAuthoritySpecV2 {
    ...
    // Delete in siriusec v8.
    repeated bytes CheckingKeys = 3 [ (gogoproto.jsontag) = "checking_keys,omitempty" ];
    repeated bytes SigningKeys = 4 [ (gogoproto.jsontag) = "signing_keys,omitempty" ];
    repeated TLSKeyPair TLSKeyPairs = 7 [ (gogoproto.nullable) = false, (gogoproto.jsontag) = "tls_key_pairs,omitempty" ];
    repeated JWTKeyPair JWTKeyPairs = 10 [ (gogoproto.nullable) = false, (gogoproto.jsontag) = "jwt_key_pairs,omitempty" ];

    // New fields in siriusec v7.
...
Auth servers need PKCS#11 configuration, passed in via `siriusec.yaml`:
...
- if no CAs exist, create a key in HSM and register it in `ActiveKeys`
  - this takes care of brand new Siriusec clusters
...
- if CAs exist and have an HSM key from this host, load the key and use it for
  ...
  - if the key is not found in the HSM (e.g. it was factory reset), the Auth
    ...
    `/var/lib/siriusec/host_uuid` to explicitly change the UUID and
...
1. HSM keys are deleted, or a new/different HSM is configured
  ...
  - The admin should remove `/var/lib/siriusec/host_uuid` to explicitly change
...
- carefully manage `siriusec.yaml` on Auth servers to ensure correct PKCS#11
  configuration
