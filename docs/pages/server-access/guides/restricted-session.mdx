...
With a Restricted Session, Siriusec allows the administrator to specify a policy to
apply to SSH sessions. This policy can restrict access to certain resources. Currently
Siriusec supports network restrictions with more types coming in the future.

## Prerequisites

Siriusec 7.0+ with Restricted Sessions requires Linux kernel 5.8 (or above).
...
## Step 1/4. Install and configure Siriusec node
...
<Admonition type="note">
  - If the Restricted Session is not enabled in `siriusec.yaml`, all network operations will be allowed.
...
## Step 3/4. Test by logging into node via Siriusec
...
```json
{
  ...
  "cluster_name": "siriusec-quickstart",
  "user": "demo",
  "login": "demo",
