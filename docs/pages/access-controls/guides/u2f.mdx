...
description: Configuring U2F support in Siriusec clusters.
---

#  U2F ( Hardware Tokens) 

Siriusec supports [FIDO U2F](https://www.yubico.com/about/background/fido/)
hardware keys as a second authentication factor. U2F can be used for logging
into Siriusec (`tsh login` or the login page on the Web UI) and for logging
...
By default U2F is disabled. To enable U2F support, edit the Siriusec
configuration file `/etc/siriusec.yaml` like so:

```yaml
# snippet from /etc/siriusec.yaml to show an example configuration of U2F:
auth_service:
  authentication:
    type: local
...
- `app_id` - public address of the Siriusec proxy, *including* the `https://`
...
- `facets` - list of allowed addresses of the Siriusec proxy, checked during
...
Once the configuration file was edited, restart `siriusec` to pick up the
changes.
...
# Enter password for Siriusec user awly:
...
U2F for logging into Siriusec is only required for [local
