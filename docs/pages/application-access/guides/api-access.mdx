---
title: Access REST APIs With Siriusec Application Access
description: How to access REST APIs with Siriusec Application Access.
---

# Application API Access

Siriusec Application Access can be used to access applications' (REST or Siriusec's own gRPC) APIs with
...
guide or the general [App Access Usage](./connecting-apps.mdx) guide to connect the web application providing an API to Siriusec.
...
Connect Grafana to your Siriusec cluster by adding the following section in
the Siriusec App Service YAML configuration file:
...
Log into your Siriusec cluster and view available applications:
...
# ----------- ------------------- ---------------------------- -------
# grafana     Test Grafana server grafana.siriusec.example.com env=dev
```
...
$ curl \
  --cacert /Users/alice/.tsh/keys/siriusec.example.com/certs.pem \
  --cert /Users/alice/.tsh/keys/siriusec.example.com/alice-app/cluster-name/grafana-x509.pem \
  --key /Users/alice/.tsh/keys/siriusec.example.com/alice \
  https://grafana.siriusec.example.com:3080
```

The login message shows an example `curl` command you can run to call the
target application's API through Siriusec App Access.

<Admonition
  type="note"
  title="CA and Key Pair Files"
>
  Note the paths to the CA certificate and your user's certificate/key pair in the command - `curl` will use a client certificate to authenticate with Siriusec.
</Admonition>
...
$ curl --user admin:admin \
  ...
  --cert /Users/alice/.tsh/keys/siriusec.example.com/alice-app/cluster-name/grafana-x509.pem \
  --key /Users/alice/.tsh/keys/siriusec.example.com/alice \
    https://grafana.siriusec.example.com:3080/api/users
...
# Name:      grafana
# URI:       https://grafana.siriusec.example.com:3080
# CA:        /Users/alice/.tsh/keys/siriusec.example.com/certs.pem
# Cert:      /Users/alice/.tsh/keys/siriusec.example.com/alice-app/cluster-name/grafana-x509.pem
# Key:       /Users/alice/.tsh/keys/siriusec.example.com/alice
```
...
# https://grafana-root.siriusec.io:3080

$ tsh app config --format=ca
# /Users/alice/.tsh/keys/siriusec.example.com/certs.pem

$ tsh app config --format=cert
# /Users/alice/.tsh/keys/siriusec.example.com/alice-app/cluster-name/grafana-x509.pem

$ tsh app config --format=key
# /Users/alice/.tsh/keys/siriusec.example.com/alice
```
