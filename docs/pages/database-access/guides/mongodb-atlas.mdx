...
description: How to configure Siriusec Database Access with MongoDB Atlas.
...
1. Configure Siriusec for accessing your MongoDB Atlas cluster.
2. Configure self-managed X.509 authentication on your Atlas cluster.
3. Connect to your Atlas cluster via Siriusec.

## Prerequisites

- Siriusec version `(=siriusec.version=)` or newer.
- [MongoDB Atlas](https://www.mongodb.com/cloud/atlas) cluster.

## Step 1/3. Configure Siriusec
...
$ siriusec db start \
  --token=/tmp/token \
  --auth-server=siriusec.example.com:443 \
  --name=mongodb-atlas \
  --protocol=mongodb \
...
Siriusec will authenticate with MongoDB Atlas using
[self-managed X.509 authentication](https://docs.atlas.mongodb.com/security-self-managed-x509/).

First, obtain Siriusec CA certificate by running the following `tctl auth sign`
command against your Siriusec cluster:
...
MongoDB cluster, say, as a user `alice`, Siriusec will sign an ephemeral
