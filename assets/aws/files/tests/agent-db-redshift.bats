...
SIRIUSEC_AGENT_DB_REDSHIFT_CLUSTER_ID=redshift-cluster-id
...
@test "[${TEST_SUITE?}] db_service.databases.aws.redshift.cluster_id is set correctly" {
    ...
    echo "${DB_DATABASES_BLOCK?}" | grep -E -A3 "^    aws:" | grep -E -A1 "^      redshift:" | grep -E "^        cluster_id: \"${SIRIUSEC_AGENT_DB_REDSHIFT_CLUSTER_ID}\""
}