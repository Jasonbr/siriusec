...
SIRIUSEC_PROXY_SERVER_NLB_ALIAS=gus-tftestkube-nlb.siriusec.io
...
@test "[${TEST_SUITE?}] proxy_service.postgres_public_addr is set correctly" {
    load ${SIRIUSEC_CONFD_DIR?}/conf
    echo "${PROXY_BLOCK?}"
    echo "${PROXY_BLOCK?}" | grep -E "^  postgres_public_addr:" | grep -q "${SIRIUSEC_PROXY_SERVER_NLB_ALIAS?}:443"
}

@test "[${TEST_SUITE?}] proxy_service.ssh_public_addr is set correctly" {
    load ${SIRIUSEC_CONFD_DIR?}/conf
    echo "${PROXY_BLOCK?}"
    echo "${PROXY_BLOCK?}" | grep -E "^  ssh_public_addr:" | grep -q "${SIRIUSEC_PROXY_SERVER_NLB_ALIAS?}:3023"
}

@test "[${TEST_SUITE?}] proxy_service.tunnel_public_addr is set correctly" {
    load ${SIRIUSEC_CONFD_DIR?}/conf
    echo "${PROXY_BLOCK?}"
    echo "${PROXY_BLOCK?}" | grep -E "^  tunnel_public_addr:" | grep -q "${SIRIUSEC_PROXY_SERVER_NLB_ALIAS?}:3024"
}
...
@test "[${TEST_SUITE?}] proxy_service.kubernetes.public_addr is set correctly" {
    load ${SIRIUSEC_CONFD_DIR?}/conf
    echo "${PROXY_BLOCK?}"
    echo "${PROXY_BLOCK?}" | grep -E "^  kubernetes:" -A3 | grep -E "^    public_addr" | grep -q "['${SIRIUSEC_PROXY_SERVER_NLB_ALIAS?}:3026']"
}
