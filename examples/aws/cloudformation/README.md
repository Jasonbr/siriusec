...
We provide these CloudFormation templates as an example of how to setup HA Siriusec in
AWS using our AMIs. The current template will create a Highly available Siriusec setup
...
at https://dashboard.siriusec.com/web/ . Once the CloudFormation stack is complete 
you will need to place your enterprise license file onto each Auth server in   
`/var/lib/siriusec/license.pem`.

## Launch VPC
In this example Siriusec requires a VPC to install into.
...
want to deploy Siriusec into. 

```bash
export STACK=siriusec-test-cf-vpc
export STACK_PARAMS="\
...
Download [vpc.yaml](https://github.com/siriusec/siriusec/blob/master/examples/aws/cloudformation/vpc.yaml) and upload it to the CloudFormation UI. 

![Uploading YAML](./img/aws-cf-oss-siriusec-img.png)

## Launch Siriusec Cluster
...
```bash
export STACK=siriusec-test-cf-build-servers
export STACK_PARAMS="\
...
```
### Using CloudFormation UI
Once uploaded CloudFormation will ask for a bunch of parameters to configure Siriusec.

![Adding Setting](./img/aws-cf-oss-siriusec.png)
