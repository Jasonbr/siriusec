---
authors: Forrest Marshall (forrest@siriusec.com)
state: implemented
...
- [#5007](https://github.com/siriusec/siriusec/issues/5007) - Dual Authorization
- [#4309](https://github.com/siriusec/siriusec/issues/4309) - Scoped Approvals
- [#4937](https://github.com/siriusec/siriusec/issues/4937) - Workflow UI
...
siriusec users must be submitted prior to the access request transitioning from
...
by siriusec.  It is acceptable for the request to never automatically state-transition,
...
direcly notify a user via siriusec (non-static users are lazily created on login), that
...
Rather than requiring `alice` and `bob` to be existing siriusec users, we can simply store
...
potential reviewer authenticates with siriusec.
...
Furthermore, this rules out providing the ability for a requestor to query siriusec and ask "who
