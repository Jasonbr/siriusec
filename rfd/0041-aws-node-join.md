---
authors: Nic Klaassen (nic@gosiriusec.com)
state: draft
---
...
Siriusec nodes running on EC2 instances will be able to join a siriusec cluster
...
https://github.com/siriusec/siriusec/blob/d4247cb150d720be97521347b74bf9c526ae869f/lib/auth/auth.go#L1538-L1563).
...
2. Check that the AWS join token matches the AWS `account` and `region` in the
   IID, and the requested Siriusec service role.
...
`ec2:DescribeInstances`. If you need Nodes to be able to join the siriusec
cluster from different AWS accounts than the one in which the Siriusec Auth
...
The design described above is not ideal for Siriusec Cloud, as the auth server
would need permissions to access customer AWS accounts to check that the
instance is currently running.

To make this work, we could create a new type of siriusec agent that the client
...
6. The auth server checks that the signed request is valid:
   ...
   - the `X-Siriusec-Challenge` header value equals the issued challenge
   - the Authorization header includes `x-siriusec-challenge` as one of the signed headers
...
8. The auth server checks for a provision token that matches the AWS account in
   the `sts:GetCallerIdentity` response and the siriusec service role of the
   join request.
...
be assumed from other accounts, see #Siriusec Configuration.
...
| Could work with Siriusec Cloud                               | ✅\* | ✅  | ✅ |
...
siriusec.yaml on the nodes should be configured so that they will use the new aws join token:
```yaml
siriusec:
