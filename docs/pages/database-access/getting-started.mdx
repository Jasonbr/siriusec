...
description: Getting started with Siriusec Database Access and AWS Aurora PostgreSQL.
---

# Getting Started

In this getting started guide we will use Siriusec Database Access to connect
...
2. Download and install Siriusec (=siriusec.version=) and connect it to the Aurora database.
3. Connect to the Aurora database via Siriusec.

## Step 1/3. Setup Aurora

In order to allow Siriusec connections to an Aurora instance, it needs to support
IAM authentication.
...
account. Siriusec Database Service will need to use credentials of this AWS user
...
## Step 2/3. Setup Siriusec

Siriusec Database Access is available starting from `6.0.0` release.

Download the appropriate version of Siriusec for your platform from
our [downloads page](https://gosiriusec.com/siriusec/download).
...
We will assume that you have configured DNS records for `siriusec.example.com` and
`*.siriusec.example.com` to point to the node where you're launching Siriusec.

Let's generate a Siriusec config with ACME enabled:

```code
$ siriusec configure --cluster-name=siriusec.example.com --acme --acme-email=alice@example.com > /tmp/siriusec.yaml
```
...
>
  Siriusec's ACME protocol integration currently requires web proxy to run on
  port 443 so open /tmp/siriusec.yaml and update `proxy_service.web_listen_addr`
  and `proxy_service.public_addr` to use port 443 instead of the default 3080.
</Admonition>

Now start Siriusec and point it to your Aurora database instance. Make sure to
update the database endpoint and region appropriately.

```code
$ sudo siriusec start --config=/tmp/siriusec.yaml \
  --roles=proxy,auth,db \
  --db-name=aurora \
...
Create the Siriusec user assigned the `db` role we've just created:
...
Now that Aurora is configured with IAM authentication, Siriusec is running and
the local user is created, we're ready to connect to the database.

Log into Siriusec with the user we've just created. Make sure to use `tsh`
version `6.0` or newer that includes Database Access support.

```code
$ tsh login --proxy=siriusec.example.com --user=alice
```
