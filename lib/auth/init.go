...
import (
	...
	"github.com/siriusec/siriusec/lib/services/local"
	"github.com/siriusec/siriusec/lib/sshca"
		...
		if err != nil {
			...
			if !asrv.keyStore.HasLocalActiveKeys(ca) && asrv.keyStore.HasLocalAdditionalKeys(ca) {
				log.Warnf("This auth server has a newly added or removed HSM and will not " +
					...
					"before routing traffic to this auth server. See https://siriusec.com/docs/admin-guide/#certificate-rotation")
			}
			if !ca.AllKeyTypesMatch() {
				log.Warnf("%s CA contains a combination of raw and PKCS#11 keys. If you are attempting to"+
					...
					" cluster and then perform a CA rotation: https://siriusec.com/docs/admin-guide/#certificate-rotation", caID.Type)
			}
		}
	}
...
func migrateLegacyResources(ctx context.Context, cfg InitConfig, asrv *Server) error {
	...
	if err := migrateCertAuthorities(ctx, asrv); err != nil {
		return trace.Wrap(err, "fail to migrate certificate authorities to the v7 storage format: %v; please report this at https://github.com/siriusec/siriusec/issues/new?assignees=&labels=bug&template=bug_report.md including the *redacted* output of 'tctl get cert_authority'", err)
	}
...
func migrateCertAuthorities(ctx context.Context, asrv *Server) error {
	...
	if len(errors) > 0 {
		...
		log.Errorf("Please report the *exact* errors below and *redacted* output of 'tctl get cert_authority' at https://github.com/siriusec/siriusec/issues/new?assignees=&labels=bug&template=bug_report.md")
