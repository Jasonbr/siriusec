...
description: How Siriusec Database Access works.
---

# Database Access Architecture

This section provides an overview of Siriusec Database Access inner workings.

## How it works

Let's take a look at a sample Database Access deployment:

![Siriusec Database Access Diagram](../../img/database-access/diagram.png)

In it, we have the following Siriusec components:

- [Siriusec Proxy](../architecture/proxy.mdx). A stateless service
  that performs a function of an authentication gateway, serves web UI and
  accepts database (and other) client connections.
- [Siriusec Auth](../architecture/authentication.mdx). Serves as
  cluster's certificate authority, handles user authentication/authorization
  and issues short-lived client certificates.
- Siriusec Database Service. The Database Access' "brain" that connects
...
6. The Database Service begins proxying traffic between the user's database
   ...
   protocol messages and submits events to the Siriusec audit log.

## Authentication

Siriusec relies on short-lived X.509 certificates for user authentication, as
...
>
  ...
  legacy terminology. In Siriusec documentation we most often refer to them as
...
Siriusec Database Service uses client certificate authentication with self-hosted
database servers.
...
Please refer to the [RFD #11](https://github.com/siriusec/siriusec/blob/024fd582e62560ffc925cd4b4d42c156043c041b/rfd/0011-database-access.md)
for a more in-depth description of the feature scope and design.

See [Architecture Overview](../architecture/overview.mdx) for general Siriusec
