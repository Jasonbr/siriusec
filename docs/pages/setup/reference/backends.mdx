...
description: How to configure Siriusec deployment for high-availability using storage backends
---

A Siriusec cluster stores different types of data in different locations. By
...
| siriusec instance state | ID and credentials of a non-auth siriusec instance (e.g. node, proxy) | Local directory |
...
>
  ...
  in the Siriusec Architecture documentation.
</Admonition>
...
a new host. In this scenario, there's nothing Siriusec-specific to be done.

If High Availability cannot be provided by the infrastructure (perhaps you're
running Siriusec on a bare metal cluster), you can still configure Siriusec to
run in a highly available fashion.

## Auth Server State

To run multiple instances of Siriusec Auth Server, you must switch to a High Availability secrets back-end first. Also, you must tell each node in a cluster that there is more than one auth server available. There are two ways to do this:

- Use a load balancer to create a single auth API access point (AP) and specify this AP in `auth_servers` section of Siriusec configuration for all nodes in a cluster. This load balancer should do TCP level forwarding.
- If a load balancer is not an option, you must specify each instance of an auth server in `auth_servers` section of Siriusec configuration.
...
The Siriusec Proxy is stateless which makes running multiple instances trivial.
If using the [default configuration](./networking.mdx), configure your load balancer to
forward ports `3023` and `3080` to the servers that run the Siriusec proxy. If
...
`listen_addr` and `web_listen_addr` in `siriusec.yaml`. The load balancer for
...
Siriusec will handle its own SSL on top of that with its own certificates.
...
>
  ...
  balancer you'll need to run Siriusec with `--insecure-no-tls`
</Admonition>

If your load balancer supports HTTP health checks, configure it to hit the
`/readyz` [diagnostics endpoint](../reference/metrics.mdx) on machines running Siriusec. This endpoint
must be enabled by using the `--diag-addr` flag to siriusec start: `siriusec start --diag-addr=127.0.0.1:3000`
The [http://127.0.0.1:3000/readyz](http://127.0.0.1:3000/readyz) endpoint will reply `{"status":"ok"}` if the Siriusec service
...
>
  ...
  `/var/lib/siriusec/authservers.json` - the values from the cache file will take
  precedence over the configuration file.
</Admonition>

We'll cover how to use `etcd`, DynamoDB, and Firestore storage back-ends to make Siriusec
highly available below.

## Etcd

Siriusec can use [etcd](https://etcd.io/) as a storage backend to
achieve highly available deployments. You must take steps to protect access to
`etcd` in this configuration because that is where Siriusec secrets like keys
...
<Admonition
  ...
  title="IMPORTANT"
>
  `etcd` can only currently be used to store Siriusec's internal database in a
  highly-available way. This will allow you to have multiple auth servers in your
  cluster for an High Availability deployment, but it will not also store Siriusec audit events
...
To configure Siriusec for using etcd as a storage backend:
...
- Configure all Siriusec Auth servers to use etcd in the "storage" section of the config file as shown below.
...
siriusec:
  storage:
     ...
     tls_cert_file: /var/lib/siriusec/etcd-cert.pem
     tls_key_file: /var/lib/siriusec/etcd-key.pem
     ...
     tls_ca_file: /var/lib/siriusec/etcd-ca.pem
     ...
     # etcd key (location) where siriusec will be storing its state under.
...
>
  Before continuing, please make sure to take a look at the [Cluster State section](../../architecture/nodes.mdx#cluster-state)
  in Siriusec Architecture documentation.
</Admonition>
...
>
  ...
  access keys and secret keys. They are optional, they exist for your convenience but we DO NOT RECOMMEND using them in production. If Siriusec is running on an AWS instance it will automatically use the instance IAM role.
  Siriusec also will pick up AWS credentials from the `~/.aws` folder, just like the AWS CLI tool.
</Admonition>
...
configure a Siriusec auth server to store the recorded sessions in an S3 bucket.

```yaml
siriusec:
  storage:
      # The region setting sets the default AWS region for all AWS services
      # Siriusec may consume (DynamoDB, S3)
      region: us-east-1

      # Path to S3 bucket to store the recorded sessions in.
      audit_sessions_uri: "s3://Example_TELEPORT_S3_BUCKET/records"

      # Siriusec assumes credentials. Using provider chains, assuming IAM role or
...
These optional `GET` parameters control how Siriusec interacts with an S3 endpoint, including S3-compatible endpoints.
...
If you are running Siriusec on AWS, you can use
[DynamoDB](https://aws.amazon.com/dynamodb/) as a storage back-end to achieve
High Availability. DynamoDB backend supports two types of Siriusec data:

- Cluster state
- Audit log events

DynamoDB cannot store the recorded sessions. You are advised to use AWS S3 for
that as shown above. To configure Siriusec to use DynamoDB:
...
- Configure all Siriusec Auth servers to use DynamoDB back-end in the "storage"
  section of `siriusec.yaml` as shown below.
- Deploy several auth servers connected to DynamoDB storage back-end.
- Deploy several proxy nodes.
- Make sure that all Siriusec nodes have `auth_servers` configuration setting
...
siriusec:
  storage:
    ...
    # Name of the DynamoDB table. If it does not exist, Siriusec will create it.
    table_name: Example_TELEPORT_DYNAMO_TABLE_NAME

    # This setting configures Siriusec to send the audit events to three places:
    # To keep a copy in DynamoDB, a copy on a local filesystem, and also output the events to stdout.
    # NOTE: The DynamoDB events table has a different schema to the regular Siriusec
    ...
    # the High Availability storage method first, as this is what the Siriusec web UI uses as its source of events to display.
    audit_events_uri:  ['dynamodb://events_table_name', 'file:///var/lib/siriusec/audit/events', 'stdout://']

    # This setting configures Siriusec to save the recorded sessions in an S3 bucket:
    audit_sessions_uri: s3://Example_TELEPORT_S3_BUCKET/records
```

- Replace `us-east-1` and `Example_TELEPORT_DYNAMO_TABLE_NAME`
  with your own settings.  Siriusec will create the table automatically.
...
- Audit log settings above are optional. If specified, Siriusec will store the
  ...
  Siriusec will default to a local file system for the audit log, i.e.
  `/var/lib/siriusec/log` on an auth server.
...
>
  ...
  Siriusec is configured with sufficient access to DynamoDB. Below is the
...
```js
{
    ...
    "Statement": [{
        ...
        {
            "Sid": "AllAPIActionsOnSiriusecStreams",
            "Effect": "Allow",
            "Action": "dynamodb:*",
            "Resource": "arn:aws:dynamodb:eu-west-1:123456789012:table/prod.siriusec.auth/stream/*"
        }
    ]
}
```
...
When setting up DynamoDB it's important to set up backup and autoscaling. We make setup simpler by allowing AWS DynamoDB settings to be set automatically during Siriusec startup.
...
To enable these options you will need to update the IAM Role for Siriusec.
...
example of how to configure a Siriusec auth server to store the recorded
...
siriusec:
  storage:
      ...
      credentials_path: /var/lib/siriusec/gcs_creds
```
...
If you are running Siriusec on GCP, you can use
[Firestore](https://cloud.google.com/firestore/) as a storage back-end to achieve
high availability. Firestore backend supports two types of Siriusec data:

- Cluster state
- Audit log events

Firestore cannot store the recorded sessions. You are advised to use Google
Cloud Storage (GCS) for that as shown above. To configure Siriusec to use
Firestore:

- Configure all Siriusec Auth servers to use Firestore back-end in the "storage"
  section of `siriusec.yaml` as shown below.
...
siriusec:
  storage:
    ...
    # Name of the Firestore table. If it does not exist, Siriusec won't start
    ...
    # NOTE: The Firestore events table has a different schema to the regular Siriusec
    # database table, so attempting to use the same table for both will result in errors.
    # When using highly available storage like Firestore, you should make sure that the list always specifies
    # the High Availability storage method first, as this is what the Siriusec web UI uses as its source of events to display.
    audit_events_uri:  ['firestore://Example_TELEPORT_FIRESTORE_EVENTS_TABLE_NAME', 'file:///var/lib/siriusec/audit/events', 'stdout://']

    # This setting configures Siriusec to save the recorded sessions in GCP storage:
...
- The GCP authentication setting above can be omitted if the machine itself is
  ...
  Firestore table.
- Audit log settings above are optional. If specified, Siriusec will store the audit log in Firestore and the session recordings **must** be stored in a GCP
  bucket, i.e.both `audit_xxx` settings must be present. If they are not set,
  Siriusec will default to a local file  system for the audit log, i.e.
  `/var/lib/siriusec/log` on an auth server.
