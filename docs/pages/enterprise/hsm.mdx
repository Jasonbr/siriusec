---
title: HSM Support
description: How to configure Hardware Security Modules to manage your Siriusec CA private keys
h1: Siriusec HSM Support
---

This section will cover:

- Setting up a Siriusec auth server to use an HSM to store and handle private
  keys.

## Prerequisites

- Siriusec v(=siriusec.version=) Enterprise self-hosted.
- An HSM reachable from your Siriusec auth server.
- The PKCS#11 module for your HSM.

While most PKCS#11 HSMs should be supported, the Siriusec team tests with AWS
...
Siriusec auth server. You should create a unique HSM user or token for Siriusec
to use.

<Tabs>
  <TabItem label="AWS CloudHSM">
Before getting started you should create and activate a CloudHSM cluster in
the VPC where you will run your Siriusec auth server. Create a Crypto User (CU)
to be used by Siriusec. See the AWS CloudHSM
[User Guide](https://docs.aws.amazon.com/cloudhsm/latest/userguide/getting-started.html)
for help.

On the EC2 instance where you will run your Siriusec auth server:
...
3. Use `yubihsm-shell` to create a new authentication key to be used by
   Siriusec with the below capabilities.
   ...
   When creating the authentication key to be used by siriusec, the password
   ...
   yubihsm> put authkey 0 0 "Siriusec Auth Key" 1 generate-asymmetric-key:sign-pkcs:delete-asymmetric-key sign-pkcs:sign-pss:decrypt-pkcs:decrypt-oaep hunter22
   Stored Authentication key 0x53bf
   ```

   Take note of the hex slot number output above, it will be used in the pin
   when configuring Siriusec.
...
5. Set the environment variable `YUBIHSM_PKCS11_CONF` to the path of your
   ...
   set in the Siriusec auth server's environment.
   ```code
...
## Step 2/5. Configure Siriusec

To configure Siriusec to use an HSM for all CA private key generation, storage,
and signing, include the `ca_key_params` section in `/etc/siriusec.yaml` on the
auth server.
...
## Step 3/5. (Re)start Siriusec Auth
...
If you are connecting an HSM to an existing Siriusec cluster, restart the auth
...
When adding a new HSM to an existing Siriusec cluster, or adding a new
HSM-connected Auth server to an HA Siriusec cluster, CA rotation needs to be
performed in order for the cluster to trust the new HSM certificates. `siriusec`
...
# wait for all siriusec servers and clients to reload and receive new certs, up
# to ~10 minutes if all clients are connected
$ tctl auth rotate --manual --phase update_servers
# wait for all siriusec servers to reload, up to ~10 minutes
...
## Step 5/5. Confirm that Siriusec is using your HSM

You are all set! Check the siriusec logs for `Creating new HSM key pair` to
