...
description: How to configure graphical database clients for Siriusec Database Access.
---

# Graphical Database Clients

This article describes how to configure popular graphical database clients to
work with Siriusec Database Access.
...
```
Name:      example
Host:      siriusec.example.com
Port:      3036
User:      alice
Database:
CA:        /Users/alice/.tsh/keys/siriusec.example.com/certs.pem
Cert:      /Users/alice/.tsh/keys/siriusec.example.com/alice-db/root/example-x509.pem
...
On the "Hostname" tab, enter your Siriusec proxy's hostname and port shown
...
and be able to validate certificate presented by your Siriusec proxy's web
...
<Admonition type="note">
  Siriusec's DBeaver MySQL integration only supports MySQL server (=mysql.dbeaver_min_ver=) or older.
</Admonition>
...
In that same tab set the username to match the one that you are connecting to using the Siriusec db certs and uncheck the 'Save password locally' box:
