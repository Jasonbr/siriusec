...
[#3169](https://github.com/siriusec/siriusec/issues/3169) raised UX concerns about how `tsh` loads keys into an SSH agent.
...
Furthermore, users have legitimate reasons for wanting to control this behavior (hence the flags). They may want to force load a certificate (our automatic detection algorithm may be incorrect) or not (the user does not want to load Siriusec certificates into their agent).

## What

By default `tsh login` should attempt to load the fetched certificate into the agent. If `tsh` thinks the agent does not support SSH certificates, like `gpg-agent`, Siriusec should not load the certificate and instead print a warning informing the user the certificate was not loaded and how to force load.
...
$ ssh-add -l
2048 SHA256:HiqSRCpljSaGt7eHCxlGGlCHcqRxSJ1vu5K7Sgl9pp4 siriusec:rjones (RSA-CERT)
2048 SHA256:HiqSRCpljSaGt7eHCxlGGlCHcqRxSJ1vu5K7Sgl9pp4 siriusec:rjones (RSA)
```
