...
description: Getting started with Siriusec. Let's deploy Siriusec in a Kubernetes with SSO and Audit logs
videoBanner: VPGYLEMTdJ8
---

# Getting Started

Let's deploy Siriusec in a Kubernetes with SSO and Audit logs:

- Install Siriusec in a Kubernetes cluster.
...
## Step 1/3. Install Siriusec

Let's start with a single-pod Siriusec using persistent volume as a backend.
...
<Tabs>
  <TabItem label="Open Source">
    ...
    # Install a single node siriusec cluster and provision a cert using ACME.
    ...
    $ helm install siriusec-cluster siriusec/siriusec-cluster --create-namespace --namespace=siriusec-cluster \
  ...
  <TabItem label="Enterprise">
    ...
    $ kubectl create namespace siriusec-cluster-ent

    # Set kubectl context to the namespace to save some typing
    $ kubectl config set-context --current --namespace=siriusec-cluster-ent

    # Get a license from Siriusec and create a secret "license" in the namespace siriusec-cluster-ent
    $ kubectl -n siriusec-cluster-ent create secret generic license --from-file=license.pem

    # Install Siriusec
    $ helm install siriusec-cluster siriusec/siriusec-cluster --namespace=siriusec-cluster-ent \
      --set clusterName=${CLUSTER_NAME?} --set acme=true --set acmeEmail=${EMAIL?} --set enterprise=true
    ```
  </TabItem>
</Tabs>

Siriusec's helm chart uses an [external load balancer](https://kubernetes.io/docs/tasks/access-application-cluster/create-external-load-balancer/)
to create a public IP for Siriusec.

<Tabs>
  <TabItem label="Open Source">
    ```code
    # Set kubectl context to the namespace to save some typing
    $ kubectl config set-context --current --namespace=siriusec-cluster

    # Service is up, load balancer is created
    $ kubectl get services
    # NAME               TYPE           CLUSTER-IP   EXTERNAL-IP      PORT(S)                        AGE
    # siriusec-cluster   LoadBalancer   10.4.4.73    104.199.126.88   443:31204/TCP,3026:32690/TCP   89s

    # Save the pod IP or Hostname. If the IP is not available, check the pod and load balancer status.
    $ MYIP=$(kubectl get services siriusec-cluster -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
    $ echo $MYIP
    # 192.168.2.1
    ```
  </TabItem>
  ...
  <TabItem label="Enterprise">
    ...
    $ kubectl get services
    # NAME                   TYPE           CLUSTER-IP   EXTERNAL-IP      PORT(S)                        AGE
    # siriusec-cluster-ent   LoadBalancer   10.4.4.73    104.199.126.88   443:31204/TCP,3026:32690/TCP   89s

    # Save the pod IP or Hostname. If the IP is not available, check the pod and load balancer status.
    $ MYIP=$(kubectl get services siriusec-cluster-ent -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
    $ echo $MYIP
    # 192.168.2.1
    ```
  </TabItem>
...
The first request to Siriusec's API will take a bit longer because it gets
a cert from [Let's Encrypt](https://letsencrypt.org).
Siriusec will respond with discovery info:
...
```code
# To create a local user, we are going to run Siriusec's admin tool tctl from the pod.
$ POD=$(kubectl get pod -l app=siriusec-cluster -o jsonpath='{.items[0].metadata.name}')
...
# NOTE: Make sure tele.example.com:443 points at a Siriusec proxy which users can access.
```
...
<Tabs>
  <TabItem label="Open Source">
    ```code
    $ curl -L -O https://get.siriusec.com/siriusec-v(=siriusec.version=)-linux-amd64-bin.tar.gz
    $ tar -xzf siriusec-v(=siriusec.version=)-linux-amd64-bin.tar.gz
    $ sudo mv siriusec/tsh /usr/local/bin/tsh
    $ sudo mv siriusec/tctl /usr/local/bin/tctl
    ```
  </TabItem>

  <TabItem label="Enterprise">
    ```code
    $ curl -L -O https://get.siriusec.com/siriusec-ent-v(=siriusec.version=)-linux-amd64-bin.tar.gz
...
```code
$ KUBECONFIG=${HOME?}/siriusec.yaml tsh login --proxy=tele.example.com:443 --user=alice
```

Siriusec updated `KUBECONFIG` with a short-lived 12-hour certificate.
...
# Once working, remove the KUBECONFIG= override to switch to siriusec
$ KUBECONFIG=${HOME?}/siriusec.yaml kubectl get -n siriusec-cluster pods
# NAME                                READY   STATUS    RESTARTS   AGE
# siriusec-cluster-6c9b88fd8f-glmhf   1/1     Running   0          127m
```
...
<Tabs>
  <TabItem label="Open Source">
    ...
    spec:
      ...
      # Map github teams to siriusec roles
      teams_to_logins:
        - organization: octocats # Github organization name
          team: admin            # Github team name within that organization
          # map github admin team to Siriusec's "admin" role
          logins: ["access"]
    ```
  </TabItem>
...
To create a connector, we are going to run Siriusec's admin tool `tctl` from the pod.

<Tabs>
  <TabItem label="Open Source">
    ```code
    $ kubectl config set-context --current --namespace=siriusec-cluster
    $ POD=$(kubectl get po -l app=siriusec-cluster -o jsonpath='{.items[0].metadata.name}')
  ...
  <TabItem label="Enterprise">
    ```code
    # To create an Okta connector, we are going to run Siriusec's admin tool tctl from the pod.
    $ POD=$(kubectl get po -l app=siriusec-cluster-ent -o jsonpath='{.items[0].metadata.name}')
...
<Tabs>
  <TabItem label="Open Source">
    ```code
    $ KUBECONFIG=${HOME?}/siriusec.yaml tsh login --proxy=tele.example.com --auth=github
    ```
  </TabItem>

  <TabItem label="Enterprise">
    ```code
    $ KUBECONFIG=${HOME?}/siriusec.yaml tsh login --proxy=tele.example.com --auth=okta
    ```
  </TabItem>
</Tabs>
...
>
  ...
  ```code
  $ kubectl exec -ti "${POD?}" -- tail -n 100 /var/lib/siriusec/log/events.log
...
- [Setup CI/CD Access with Siriusec](../guides/cicd.mdx)
