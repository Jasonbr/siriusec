...
description: Forwarding events with Fluentd and Siriusec event handler
---

This section will cover:

- Setting up Siriusec's event handler.
- Forwarding events with Fluentd for Cloud, Enterprise and Open Source editions.

## Prerequisites

- Siriusec v(=siriusec.version=) Cloud, Open Source or Enterprise
- Siriusec admin tool `tctl` configured to access the cluster.
- Fluentd version v(=fluentd.version=).
- Docker version v(=docker.version=).

```code
$ tctl version
# Siriusec v(=siriusec.version=) go(siriusec.golang)
#
...
Siriusec event handler runs alongside fluentd forwarder, receives events from Siriusec's events API
and forwards them to fluentd.

<Tabs>
  <TabItem label="Linux">
  ```code
  $ curl -L -O https://get.siriusec.com/siriusec-event-handler-v(=siriusec.version=)-linux-amd64-bin.tar.gz
  $ tar -zxvf siriusec-event-handler-v(=siriusec.version=)-linux-amd64-bin.tar.gz
  ```
  </TabItem>

  <TabItem label="MacOS">
  ```code
  $ curl -L -O https://get.siriusec.com/siriusec-event-handler-v(=siriusec.version=)-darwin-amd64-bin.tar.gz
  $ tar -zxvf siriusec-event-handler-v(=siriusec.version=)-linux-amd64-bin.tar.gz
  ```
  </TabItem>
</Tabs>
...
Run `configure` command to generate sample configuration. Replace `siriusec.example.com:443`
with the DNS name and web proxy port of Siriusec's proxy:

```code
$ ./siriusec-event-handler configure . siriusec.example.com:443
```

You'll see the following output:

```txt
Siriusec event handler 0.0.1 07617b0ad0829db043fe779faf1669defdc8d84e

[1] mTLS Fluentd certificates generated and saved to ca.crt, ca.key, server.crt, server.key, client.crt, client.key
[2] Generated sample siriusec-event-handler role and user file siriusec-event-handler-role.yaml
[3] Generated sample fluentd configuration file fluent.conf
[4] Generated plugin configuration file siriusec-event-handler.toml

Follow-along with our getting started guide:

https://gosiriusec.com/setup/guides/fluentd
```
...
# -rw------- 1 bob bob      260 Jul  1 11:14 siriusec-event-handler-role.yaml
# -rw------- 1 bob bob      343 Jul  1 11:14 siriusec-event-handler.toml
```
...
- `siriusec-event-handler-role.yaml` is a Siriusec's event handler client user and role.
- `fluent.conf` is a fluentd plugin configuration.

## Step 3/6. Create user and role for reading audit events

`configure` command generated `siriusec-event-handler-role.yaml` that defines a `siriusec-event-handler` role and a user with read-only access to the `event` API:

```yaml
kind: user
metadata:
  name: siriusec-event-handler
spec:
  roles: ['siriusec-event-handler']
version: v2
---
kind: role
metadata:
...
```code
$ tctl create -f siriusec-event-handler-role.yaml
# user "siriusec-event-handler" has been created
# role 'siriusec-event-handler' has been created
```

## Step 4/6. Create siriusec-event-handler credentials

Siriusec Plugin use the fluentd role and user to read the events. We export the identity files, using tctl auth sign.

```code
$ tctl auth sign --out identity --user siriusec-event-handler
# The credentials have been written to identity
```

This will generate `identity` which contains TLS certificates and will be used to connect plugin to your Siriusec instance.
...
Create the following file with role: `siriusec-event-handler-impersonator.yaml`:

```yaml
kind: role
version: v4
metadata:
  name: siriusec-event-handler-impersonator
spec:
...
```code
$ tctl create siriusec-event-handler-impersonator.yaml
```
...
Now edit `out.yaml` with your favourite editor, adding `siriusec-event-handler-impersonator` to the list of existing roles and update via `tctl` again:
```
$ tctl create -f out.yaml
```

Re-login to assume the new role and try to issue certificate for the siriusec-event-handle user again.
</Admonition>
...
`configure` command generated `siriusec-event-handler.toml` configuration file
...
```code
$ ./siriusec-event-handler start --config siriusec-event-handler.toml
```
...
```code
$ ./siriusec-event-handler start --config siriusec-event-handler.toml --start-time "2021-05-05T00:00:00Z"
```
...
- Read more about [impersonation](https://gosiriusec.com/docs/access-controls/guides/impersonation/) here.
