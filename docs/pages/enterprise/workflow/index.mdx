...
description: How to customize SSH and Kubernetes access using Siriusec.
h1: Siriusec Access Requests
---

#### Approving Requests using an External Integration

- [Integrating Siriusec with Slack](ssh-approval-slack.mdx)
- [Integrating Siriusec with Mattermost](ssh-approval-mattermost.mdx)
- [Integrating Siriusec with Jira Cloud](ssh-approval-jira-cloud.mdx)
- [Integrating Siriusec with Jira Server](ssh-approval-jira-server.mdx)
- [Integrating Siriusec with PagerDuty](ssh-approval-pagerduty.mdx)

## Access Requests Setup

Siriusec has the ability for users to request additional roles. The
...
spec:
  allow:
    ...
    # `access_request` should only be given to Siriusec Admins.
    # For a full list of allow-rules, see
    # https://gosiriusec.com/docs/access-controls/reference/
    - resources:
      - access_request
      verbs:
      - list
...
```code
$ tsh login siriusec-cluster --request-roles=dba
    ...
    request:
      ...
      claims_to_roles:
        - claim: groups
          value: admins
          roles: ['*']
      # Siriusec can attach annotations to pending access requests. these
...
>
  Siriusec RBAC offers powerful wildcard and RegEx helpers. Below are a few examples.
...
<Admonition
  ...
  title="Note"
>
  Notice that the above role does not specify any logins. If a users's roles specify no logins, Siriusec will now generate the user's initial SSH certificates with an invalid dummy login of the form `-siriusec-nologin-<random-suffix>` (e.g. `-siriusec-nologin-1e02dbfd`).
</Admonition>
...
- Approved requests have no effect on Siriusec's behavior outside of allowing additional
  roles on re-issue. This has the nice effect of making requests "compatible" with
  older versions of Siriusec, since only the issuing Auth Server needs any particular
