---
title: Running an HA Siriusec cluster using AWS, EKS, and Helm
description: Install and configure an HA Siriusec cluster using an AWS EKS cluster
---

In this guide, we'll go through how to set up a High Availability Siriusec cluster with multiple replicas in Kubernetes
using Siriusec Helm charts and AWS products (DynamoDB and S3).

(!docs/pages/kubernetes-access/helm/includes/helm-install.mdx!)

## Step 3. Set up AWS IAM configuration

For Siriusec to be able to create the DynamoDB tables, indexes, and the S3 storage bucket it needs,
...
| `siriusec-helm-backend` | DynamoDB table name to use for the Siriusec backend |
| `siriusec-helm-events` | DynamoDB table name to use for the Siriusec audit log (**must** be different to the backend table)
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:dynamodb:us-west-2:1234567890:table/siriusec-helm-backend",
                "arn:aws:dynamodb:us-west-2:1234567890:table/siriusec-helm-backend/stream/*"
            ]
        },
        {
            ...
            "Resource": [
                "arn:aws:dynamodb:us-west-2:1234567890:table/siriusec-helm-events",
                "arn:aws:dynamodb:us-west-2:1234567890:table/siriusec-helm-events/index/*"
            ]
        }
    ]
}
...
| Placeholder value | Replace with |
| - | - |
| `siriusec-helm-sessions` | Name to use for the Siriusec S3 session recording bucket |
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:s3:::siriusec-helm-sessions/*",
                "arn:aws:s3:::siriusec-helm-sessions"
            ]
        }
    ]
...
This policy allows `cert-manager` to use DNS01 Let's Encrypt challenges to provision TLS certificates for your Siriusec cluster.

You'll need to replace these values in the policy example below:

| Placeholder value | Replace with |
| - | - |
| `Z0159221358P96JYAUAA4` | Route 53 hosted zone ID for the domain hosting your Siriusec cluster |
...
| `example.com` | The name of the Route 53 domain hosting your Siriusec cluster |
...
$ kubectl --namespace siriusec create -f aws-issuer.yaml
```

## Step 5. Set values to configure the cluster

There are two different ways to configure the `siriusec-cluster` Helm chart to use `aws` mode - using a `values.yaml` file, or using `--set`
on the command line.
...
<Tabs>
  ...
  aws:
    ...
    backendTable: siriusec-helm-backend             # DynamoDB table to use for the Siriusec backend
    auditLogTable: siriusec-helm-events             # DynamoDB table to use for the Siriusec audit log (must be different to the backend table)
    sessionRecordingBucket: siriusec-helm-sessions  # S3 bucket to use for Siriusec session recordings
  ...
  $ helm install siriusec siriusec/siriusec-cluster \
    ...
    --set aws.backendTable=siriusec-helm-backend                          `# DynamoDB table to use for the Siriusec backend` \
    --set aws.backups=true                                                `# Whether or not to turn on DynamoDB backups` \
    --set aws.auditLogTable=siriusec-helm-events                          `# DynamoDB table to use for the Siriusec audit log (must be different to the backend table)` \
    --set aws.sessionRecordingBucket=siriusec-helm-sessions               `# S3 bucket to use for Siriusec session recordings` \
...
<Admonition type="note">
  You cannot change the `clusterName` after the cluster is configured, so make sure you choose wisely. You should use the fully-qualified domain name that you'll use for external access to your Siriusec cluster.
</Admonition>
...
# NOTE: Make sure siriusec.example.com:443 points at a Siriusec proxy that users can access.
```
