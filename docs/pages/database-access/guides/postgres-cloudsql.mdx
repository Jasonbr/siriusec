...
description: How to configure Siriusec Database Access with GCP Cloud SQL PostgreSQL.
...
Siriusec uses [IAM database authentication](https://cloud.google.com/sql/docs/postgres/authentication)
...
Siriusec uses service accounts to connect to Cloud SQL databases.
...
## Create service account for Siriusec Database service

The final part of GCP configuration is to create a service account for the
Siriusec Database service with the following IAM roles:
...
Save the file, your Siriusec Database Service will need it to be able to generate
IAM auth tokens.

## Gather Cloud SQL instance information

To connect a Cloud SQL database to Siriusec, you'll need to gather a few pieces
...
The instance's root certificate is required so Siriusec can validate the
...
## Setup Siriusec Auth and Proxy services

Siriusec Database Access for Cloud SQL PostgreSQL is available starting from
`6.2` release.
...
Create local Siriusec user with the built-in `access` role:
...
connect to the `postgres` database as a `siriusec@<project-id>.iam` database
service account.
...
## Setup Siriusec Database service
...
<Admonition
  ...
  title="Tip"
>
  A single Siriusec process can run multiple different services, for example
...
```code
$ siriusec start --config=/path/to/siriusec-db.yaml --token=/tmp/token
```

### GCP credentials

Siriusec Database Service must have credentials of `siriusec-db-service` GCP
service account we created [above](#create-service-account-for-siriusec-database-service)
...
Siriusec using `tsh db login` command:
...
```code
$ tsh db login --db-user=siriusec@<project-id>.iam --db-name=postgres cloudsql
```
