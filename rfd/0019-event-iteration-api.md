---
authors: Joel Wejdenstal (jwejdenstal@gosiriusec.com)
state: implemented
---
...
This RFC was sparked from issue [#5435](https://github.com/siriusec/siriusec/issues/5435).
...
The current API for event fetch and search is over (https://github.com/siriusec/siriusec/blob/master/lib/auth/apiserver.go#L246) plain HTTPS and not GRPC and runs on the authentication server.
...
These should be defined in [api/client/proto/authservice.proto](https://github.com/siriusec/siriusec/blob/master/api/client/proto/authservice.proto).
...
Currently the DynamoDB implementation is incorrect in the HTTP API as described in [#5435](https://github.com/siriusec/siriusec/issues/5435). This should be fixed so that event filtering works correctly in the new GRPC API although I'd like some discussion on if it should be fixed in the current API or should be left as is.
...
These endpoints will live on the `AuthService` GRPC service in [api/client/proto/authservice.proto](https://github.com/siriusec/siriusec/blob/master/api/client/proto/authservice.proto) and are defined as follows:
