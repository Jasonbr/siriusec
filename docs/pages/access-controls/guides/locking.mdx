...
Siriusec provides a locking mechanism to restrict access to a computing
...
- a Siriusec user by the user's name;
- a Siriusec [RBAC](../reference.mdx) role by the role's name;
- an MFA device by the device's UUID;
- an OS/UNIX login;
- a Siriusec node by the node's UUID (effectively unregistering it from the
  cluster).

<Admonition type="warning">
  Locking support was introduced in Siriusec v7.1. To fully enforce the locks
  at all components, you should update **all** `siriusec` binaries in your
  deployment.
</Admonition>
...
If a Siriusec node or proxy cannot properly synchronize its local lock view
...
<Tabs>
  ...
  <TabItem label="Static Config">
    Edit the `siriusec.yaml` of the Auth server:
