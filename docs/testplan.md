...
Below are the items that should be manually tested with each release of Siriusec.
These tests should be run on both a fresh install of the version to be released
as well as an upgrade of the previous version of Siriusec.
...
- [ ] Backends
  - [ ] Siriusec runs with etcd
  - [ ] Siriusec runs with dynamodb
  - [ ] Siriusec runs with boltdb
  - [ ] Siriusec runs with dir
...
- [ ] Connect to a OpenSSH node in a local cluster using Siriusec.
- [ ] Connect to a OpenSSH node in a local cluster using the Web UI.
- [ ] Connect to a Siriusec node in a local cluster using OpenSSH.
- [ ] Connect to a Siriusec node in a local cluster using Siriusec.
- [ ] Connect to a Siriusec node in a local cluster using the Web UI.
- [ ] Connect to a OpenSSH node in a remote cluster using OpenSSH.
- [ ] Connect to a OpenSSH node in a remote cluster using Siriusec.
- [ ] Connect to a OpenSSH node in a remote cluster using the Web UI.
- [ ] Connect to a Siriusec node in a remote cluster using OpenSSH.
- [ ] Connect to a Siriusec node in a remote cluster using Siriusec.
- [ ] Connect to a Siriusec node in a remote cluster using the Web UI.

### Siriusec with EKS/GKE

* [ ] Deploy Siriusec on a single EKS cluster
* [ ] Deploy Siriusec on two EKS clusters and connect them via trusted cluster feature
* [ ] Deploy Siriusec Proxy outside of GKE cluster fronting connections to it (use [this script](https://github.com/siriusec/siriusec/blob/master/examples/k8s-auth/get-kubeconfig.sh) to generate a kubeconfig)
* [ ] Deploy Siriusec Proxy outside of EKS cluster fronting connections to it (use [this script](https://github.com/siriusec/siriusec/blob/master/examples/k8s-auth/get-kubeconfig.sh) to generate a kubeconfig)

### Siriusec with multiple Kubernetes clusters
...
### Siriusec with FIPS mode

* [ ] Perform trusted clusters, Web and SSH sanity check with all siriusec components deployed in FIPS mode.
...
- [ ] Test receiving a message via Siriusec Slackbot
- [ ] Test receiving a new Jira Ticket via Siriusec Jira
...
Create/modify `siriusec.yaml` and set the following authentication settings under `auth_service`
```
authentication:
  type: local
...
```
tsh bench --duration=4h user@siriusec-monster-6757d7b487-x226b ls
tsh bench -i --duration=4h user@siriusec-monster-6757d7b487-x226b ps uax
```
...
## Siriusec with Cloud Providers

### AWS

- [ ] Deploy Siriusec to AWS. Using DynamoDB & S3
- [ ] Deploy Siriusec Enterprise to AWS. Using HA Setup https://siriusec.com/siriusec/docs/aws-terraform-guide/

### GCP

- [ ] Deploy Siriusec to GCP. Using Cloud Firestore & Cloud Storage
- [ ] Deploy Siriusec to GKE. Google Kubernetes engine.
- [ ] Deploy Siriusec Enterprise to GCP.

### IBM

- [ ] Deploy Siriusec to IBM Cloud. Using IBM Database for etcd & IBM Object Store
- [ ] Deploy Siriusec to IBM Cloud Kubernetes.
- [ ] Deploy Siriusec Enterprise to IBM Cloud.
...
- [ ] Verify JWT using [verify-jwt.go](https://github.com/siriusec/siriusec/blob/master/examples/jwt/verify-jwt.go).
- [ ] Verify RBAC.
- [ ] Verify [CLI access](https://gosiriusec.com/docs/application-access/guides/api-access/) with `tsh app login`.
