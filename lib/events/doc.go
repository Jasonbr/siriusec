...
Print event specifies session output - PTY io recorded by Siriusec node or Proxy
...
The main log files are saved as:
	...
	/var/lib/siriusec/log/<auth-server-id>/<date>.log
...
Index file contains a list of event files and chunks files associated with a session:
	...
	/var/lib/siriusec/log/sessions/<auth-server-id>/<session-id>.index
...
Files:
	...
	/var/lib/siriusec/log/<auth-server-id>/<session-id>-<first-event-in-file-index>.events
	/var/lib/siriusec/log/<auth-server-id>/<session-id>-<first-chunk-in-file-offset>.chunks
...
File with session events /var/lib/siriusec/a1/s1-0.events will contain:
...
File with recorded session /var/lib/siriusec/a1/s1-0.chunks will contain 40 bytes
...
Both auth servers have mounted /var/lib/siriusec/log as a shared NFS folder.
...
Events file /var/lib/siriusec/a1/s1-0.events will contain:

{"ei":0,"event":"session.start", ...}
{"ei":1,"event":"resize",...}
{"ei":2,"ci":0, "event":"print","bytes":40,"offset":0}

Events file /var/lib/siriusec/a1/s1-0.chunks will contain 40 bytes
emitted by print event with chunk index.

Server a2 will produce the following file:

/var/lib/siriusec/a2/s1.index
...
Events file /var/lib/siriusec/a2/s1-4.events will contain:

{"ei":3,"ci":1, "event":"print","bytes":15,"ms":713,"offset":40}
{"ei":4,"event":"session.end", ...}

Events file /var/lib/siriusec/a2/s1-40.chunks will contain 15 bytes emitted
