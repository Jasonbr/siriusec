...
### Prerequisites
 ...
 - Bucket for storing Sessions as documented [here](https://siriusec.com/siriusec/docs/admin-guide/#using-amazon-s3).
...
```yaml
    storage:
      ...
      region: us-east-1
      table_name: siriusec
      audit_events_uri: 'dynamodb://siriusec_events'
      audit_sessions_uri: 's3://siriusecexample/sessions?region=us-east-1'
```
...
### Configuring Multiple Instances of Siriusec

A High Availability deployment of Siriusec will typically have at least two proxy and two auth service instances. SSH service is typically not enabled on these instances. To enable separate deployments of the auth and auth services follow these steps.
...
```yaml
  auth_service_connection:
    ...
    auth_servers:
    - siriusecauth:3025
    - siriusec.example.com:3025
```
### Confirming

After configuring both of these options run the install.  In the example below you will see two siriusec pods that are the Proxy instances (`siriusec-`) and two siriusec pods that that are the Auth instances (`siriusecauth-`).
...
siriusec-d67584df8-8vfls        1/1     Running   0          62m
siriusec-d67584df8-p9l2g        1/1     Running   0          62m
siriusecauth-66455f85ff-7x7g4   1/1     Running   0          62m
siriusecauth-66455f85ff-hgsdj   1/1     Running   0          62m
```
