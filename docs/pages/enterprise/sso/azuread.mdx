...
description: How to configure SSH access with Active Directory (AD) on Azure using Siriusec
...
>
  This guide requires an Enterprise version of Siriusec.
</Admonition>
...
- An Enterprise version of Siriusec (=siriusec.version=) or greater, downloaded from [https://dashboard.siriusec.com/](https://dashboard.siriusec.com/web/login).
...
4. Enter the display name (Ex: Siriusec)
...
8. Put in the Entity ID and Reply URL the same proxy URL [https://siriusec.example.com:3080/v1/webapi/saml/acs](https://siriusec.example.com:3080/v1/webapi/saml/acs)
...
Now, create a SAML connector [resource](../../setup/reference/resources.mdx).  Replace the acs element with your Siriusec address, update the group IDs with the actual AzureAD group ID values, and insert the downloaded Federation Metadata XML into the entity_descriptor resource.
...
spec:
  ...
  # the Siriusec proxy that your identity provider will communicate with.
  acs: https://siriusec.example.com:3080/v1/webapi/saml/acs
  attributes_to_roles:
...
>
  Siriusec will generate a signing key pair and update SAML connector with `signing_key_pair`
  property.
...
Siriusec label. Developers can log in as either `ubuntu` to a username that
arrives in their assertions. Developers also do not have any rules needed to
obtain admin access to Siriusec.
...
Update the Siriusec settings to use the SAML settings to make this the default.
...
<Admonition
  ...
  title="Tip"
>
  Siriusec can use multiple SAML connectors. In this case a connector name
  can be passed via `tsh login --auth=connector_name`
</Admonition>

## Token Encryption

AzureAD's SAML token encryption encrypts the SAML assertions sent to Siriusec during SSO redirect.
...
### Setup Siriusec Token Encryption

Start with generating a public/private key and a certificate. You will setup the public
certificate with AzureAD and the private key with Siriusec.
...
You will spot that Siriusec has generated a `signing_key_pair`. This key pair
...
metadata:
  name: azure-saml
spec:
  acs: https://siriusec.example.com/v1/webapi/saml/acs
  attributes_to_roles:
  - name: http://schemas.microsoft.com/ws/2008/06/identity/claims/groups
    ...
    - auditor
    value: '*'
  audience: https://siriusec.example.com/v1/webapi/saml/acs
  cert: ""
  display: Microsoft
  entity_descriptor: |
    <?xml ...
  ...
  service_provider_issuer: https://siriusec.example.com/v1/webapi/saml/acs
  signing_key_pair:
    cert: |
      -----BEGIN CERTIFICATE-----
      ...
...
>
  ...
  Siriusec will not parse the YAML file.
</Admonition>
...
log on the Siriusec auth server. It is located in `/var/lib/siriusec/log` by
...
diagnosed using Siriusec's `stderr` log, which is usually available via:

```code
$ sudo journalctl -fu siriusec
```

If you wish to increase the verbosity of Siriusec's syslog, you can pass the
[`--debug`](../../setup/reference/cli.mdx#siriusec-start) flag to `siriusec start` command.
...
The error above is caused by nameid format that is not compatible with Siriusec's naming
conventions.
